...
class EncoderHeadTrainer:
    def __init__(
        ...
        if opt_type == "adam":
            self.optimizer = torch.optim.Adam(
                params, lr=lr, weight_decay=opt_config.get("weight_decay", 0.0)
            )
        elif opt_type == "adamw":
            self.optimizer = torch.optim.AdamW(
                params, lr=lr, weight_decay=opt_config.get("weight_decay", 0.01)
            )
        elif opt_type == "sgd":
        ...
        self.best_val_metric = (
            float("inf") if self._is_lower_better() else float("-inf")
        )
    ...
    def train(self):
        ...
        test_metrics = evaluate_model(
            self.model, self.test_loader, self.task_type, self.task_config, self.device
        )
    ...
    def _validate(self):
        ...
        metrics = evaluate_model(
            self.model, self.val_loader, self.task_type, self.task_config, self.device
        )
    ...
    def _log_metrics(
        self, train_loss: float, val_metrics: Dict[str, float], epoch: int
    ):
        # tb
        ...
        if self.task_type == "pattern_classification":
            logger.info(
                f"  Val Metrics - Hamming: {val_metrics.get('hamming_loss', 0):.4f}, "
                f"Subset Acc: {val_metrics.get('subset_accuracy', 0):.4f}, "
                f"Macro F1: {val_metrics.get('macro_f1', 0):.4f}"
            )
        elif self.task_type == "accuracy_prediction":
            logger.info(
                f"  Val Metrics - MSE: {val_metrics.get('mse', 0):.4f}, "
