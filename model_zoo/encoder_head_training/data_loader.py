...
class EncoderHeadDataset(Dataset):
    def __init__(self, dataset_repo_id: str, tokenizer, task_type: str,
                 task_config: Dict[str, Any], split: str = 'train',
                 cache_dir: Optional[Path] = None, encoder: Optional[torch.nn.Module] = None,
                 device: str = 'cpu'):
    ...
    def _precompute_representations(self, encoder: torch.nn.Module):
        ...
        logger.info(f"Cached representations shape: {self.cached_representations.shape}")
    ...
    def _prepare_target(self, sample: Dict[str, Any]):
        ...
        if self.task_type == 'pattern_classification':
            ...
            for name, config in continuous_targets.items():
                ...
                targets[f'continuous_{name}'] = torch.tensor([value], dtype=torch.float32)
...
def create_data_loaders(dataset_repo_id: str, tokenizer, task_type: str,
                       task_config: Dict[str, Any], batch_size: int,
                       cache_dir: Optional[Path] = None,
                       encoder: Optional[torch.nn.Module] = None,
                       device: str = 'cpu', num_workers: int = 0):
    train_dataset = EncoderHeadDataset(
        dataset_repo_id, tokenizer, task_type, task_config,
        split='train', cache_dir=cache_dir, encoder=encoder, device=device
    )
    val_dataset = EncoderHeadDataset(
        dataset_repo_id, tokenizer, task_type, task_config,
        split='validation', cache_dir=cache_dir, encoder=encoder, device=device
    )
    test_dataset = EncoderHeadDataset(
        dataset_repo_id, tokenizer, task_type, task_config,
        split='test', cache_dir=cache_dir, encoder=encoder, device=device
    )
