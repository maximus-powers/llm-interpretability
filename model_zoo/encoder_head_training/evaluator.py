...
from sklearn.metrics import (
    ...
    hamming_loss, roc_auc_score, mean_absolute_error,
...
class EncoderHeadEvaluator:
    ...
    def _evaluate_pattern_classification(self, logits: torch.Tensor, targets: torch.Tensor):
        ...
        precision, recall, f1, support = precision_recall_fscore_support(targets_np, predictions, average=None, zero_division=0)
        metrics['micro_f1'] = precision_recall_fscore_support(targets_np, predictions, average='micro', zero_division=0)[2]
        metrics['macro_f1'] = precision_recall_fscore_support(targets_np, predictions, average='macro', zero_division=0)[2]
        metrics['macro_precision'] = precision_recall_fscore_support(targets_np, predictions, average='macro', zero_division=0)[0]
        metrics['macro_recall'] = precision_recall_fscore_support(targets_np, predictions, average='macro', zero_division=0)[1]
        try:
    ...
    def _evaluate_accuracy_prediction(self, predictions: torch.Tensor, targets: torch.Tensor):
    ...
    def _evaluate_hyperparameter_prediction(self, predictions: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]):
        metrics = {}
        ...
        for name, config in discrete_targets.items():
            ...
            # classwise metrics
            precision, recall, f1, _ = precision_recall_fscore_support(target, pred_classes, average='macro', zero_division=0)
...
def evaluate_model(model: torch.nn.Module, data_loader, task_type: str, task_config: Dict[str, Any], device: str = 'cpu'):
    model.eval()
