...
def _build_mlp(input_dim: int, hidden_dims: List[int], dropout: float, activation: str, batch_norm: bool = False):
    activation_map = {
        'relu': nn.ReLU(),
        'gelu': nn.GELU(),
        'tanh': nn.Tanh(),
...
class PatternClassificationHead(nn.Module):
    def __init__(self, latent_dim: int, num_patterns: int, hidden_dims: List[int], dropout: float, activation: str, batch_norm: bool = False):
        super().__init__()
        self.latent_dim = latent_dim
        self.num_patterns = num_patterns
        if hidden_dims:
            self.mlp = _build_mlp(latent_dim, hidden_dims, dropout, activation, batch_norm)
...
class AccuracyPredictionHead(nn.Module):
    def __init__(self, latent_dim: int, hidden_dims: List[int], dropout: float, activation: str, batch_norm: bool = False):
        super().__init__()
        self.latent_dim = latent_dim
...
class HyperparameterPredictionHead(nn.Module):
    def __init__(self, latent_dim: int, hidden_dims: List[int], continuous_targets: Dict[str, Dict], 
                 discrete_targets: Dict[str, Dict], dropout: float, activation: str, batch_norm: bool = False):
        super().__init__()
        self.latent_dim = latent_dim
        ...
        if hidden_dims:
            self.shared_mlp = _build_mlp(latent_dim, hidden_dims, dropout, activation, batch_norm)
            final_dim = hidden_dims[-1]
        else:
        ...
        for name, config in continuous_targets.items():
            self.continuous_heads[name] = nn.Sequential(
                nn.Linear(final_dim, 1)
            )
...
def create_prediction_head(task_type: str, latent_dim: int, task_config: Dict, model_config: Dict):
    ...
    if task_type == 'pattern_classification':
        ...
        logger.info(f"Creating HyperparameterPredictionHead with {len(continuous_targets)} continuous and {len(discrete_targets)} discrete targets")
