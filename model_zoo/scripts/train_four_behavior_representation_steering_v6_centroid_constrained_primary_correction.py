"""Four-behavior centroid-constrained representation steering under V6 preregistration."""
...
DEFAULT_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_representation_steering_v6_pools"
TRAINING_CONFIG = {
    ...
    "transport_method": "train_centroid_constrained_primary_correction",
...
THRESHOLDS = {
    ...
    "min_mean_matched_minus_best_control_centroid_improvement": 0.05,
    "min_mean_matched_minus_best_control_primary_target_margin": 0.10,
    "min_mean_matched_minus_v2_centroid_delta_centroid_improvement": 0.05,
    "min_mean_matched_minus_v2_centroid_delta_primary_target_margin": 0.10,
    "min_mean_matched_minus_v3_diagonal_transport_centroid_improvement": 0.05,
    ...
    "min_mean_matched_minus_v5_calibrated_centroid_improvement": 0.05,
    "min_mean_matched_minus_v5_calibrated_primary_target_margin": 0.10,
    ...
    "min_per_record_matched_minus_v5_calibrated_centroid_improvement": 0.0,
    "min_per_record_matched_minus_v5_calibrated_primary_target_margin": 0.0,
...
V6_FINAL_RAW = DEFAULT_POOL_DIR / "final_subjects.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="runs/four_behavior_representation_steering_v6_centroid_constrained_primary_correction",
    )
...
def validate_preregistered_args(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise ValueError("Non-preregistered V6 parameter override: " + "; ".join(failures))


def assert_preregistered_pool_dir(pool_dir: Path) -> None:
    resolved = pool_dir.resolve()
    if resolved != DEFAULT_POOL_DIR.resolve():
        raise ValueError(f"V6 requires preregistered pool directory: {DEFAULT_POOL_DIR}")
    for forbidden in (
        V1_POOL_DIR.resolve(),
        V2_POOL_DIR.resolve(),
        V3_POOL_DIR.resolve(),
        ...
        if resolved == forbidden or forbidden in resolved.parents:
            raise ValueError(f"V6 may not use prior steering source pools: {resolved}")
    ...
    *,
    ...
    for raw_path in paths:
        ...
        if path.name != "final_subjects.json":
            continue
        if allow_v6_final and path == V6_FINAL_RAW.resolve():
            continue
        raise ValueError(f"V6 steering final raw path is forbidden before final eval: {path}")
...
def generate_pools(args: argparse.Namespace, pool_dir: Path) -> Dict[str, Any]:
    ...
    for pool_name, pool_config in POOL_CONFIGS.items():
        ...
        payload["claim_scope"] = "four_behavior_representation_steering_v6_source_pool"
    ...
    final_redacted["claim_scope"] = "redacted_final_steering_v6_source_pool_audit_surface_only"
    ...
    audit["claim_scope"] = "four_behavior_representation_steering_v6_source_pool_construction"
    audit = redact_v6_combined_audit(audit)
...
def redact_v6_combined_audit(audit: Mapping[str, Any]) -> Dict[str, Any]:
...
def run_development(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths(paths.values(), allow_v6_final=False)
...
def run_final(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths([eval_path], allow_v6_final=True)
    payload = train_and_evaluate(
...
def validate_development_authorizes_final(
    *,
    ...
    failures = []
    expected = {
        "claim_scope": "four_behavior_representation_steering_v6_centroid_constrained_primary_correction_development",
...
def train_and_evaluate(
    *,
    ...
    assert_no_forbidden_final_raw_paths(
        ...
        allow_v6_final=allow_eval_final_raw,
    )
    ...
    if hash_failures:
        raise ValueError("V6 source-pool file hash validation failed: " + "; ".join(hash_failures))
    ...
    if contract_failures:
        raise ValueError("V6 source-pool contract validation failed: " + "; ".join(contract_failures))
    ...
    transport_path = output_dir / "centroid_constrained_primary_correction_stats.pt"
    torch.save(
        {
            ...
            "method": "train_centroid_constrained_primary_correction",
    ...
    result = {
        **eval_result,
        ...
        "claim_scope": (
            ...
            else "four_behavior_representation_steering_v6_centroid_constrained_primary_correction_final"
        ),
...
def fit_train_statistics(subjects: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    if vh.size(0) < rank:
        raise ValueError(f"V6 requires at least {rank} principal components")
...
def evaluate_record(
    *,
    ...
    summary = {
        ...
        "matched_minus_v5_calibrated_centroid_improvement": (
            matched["centroid_improvement"] - v5_control["centroid_improvement"]
        ),
        "matched_minus_v5_calibrated_primary_target_margin": (
            matched["primary_target_margin"] - v5_control["primary_target_margin"]
        ),
...
def build_control_candidate_specs(
    *,
    ...
    specs: list[Dict[str, Any]] = [
        ...
        candidate_spec(
            "reverse_v6_centroid_constrained_primary_correction",
    ...
    for other_target in PATTERNS:
        ...
        specs.append(candidate_spec(
            "same_source_other_target_v6_centroid_constrained_primary_correction",
    ...
    for other_source in PATTERNS:
        if other_source in (source, target):
            continue
        specs.append(candidate_spec(
            "same_target_other_source_v6_centroid_constrained_primary_correction",
    ...
    specs.append(candidate_spec(
        "shuffled_v6_centroid_constrained_primary_correction",
...
def select_shuffled_direction(
    ...
    digest = stable_hash_json([
        ...
        source,
        target,
        "representation_steering_v6_centroid_constrained_primary_correction_shuffled_direction",
    ])
...
def random_norm_matched_vectors(
    *,
    ...
    seed_digest = stable_hash_json([
        ...
        source,
        target,
        "representation_steering_v6_centroid_constrained_primary_correction_random",
    ])
...
def summarize_records(records: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "mean_matched_minus_v5_calibrated_centroid_improvement": v1.mean([
            item["matched_minus_v5_calibrated_centroid_improvement"]
            for item in summaries
        ]),
        "mean_matched_minus_v5_calibrated_primary_target_margin": v1.mean([
            item["matched_minus_v5_calibrated_primary_target_margin"]
            for item in summaries
        ]),
...
def individual_gate_audit(records: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    for record in records:
        ...
        checks = [
            ...
            (
                "matched_minus_v5_calibrated_primary_target_margin",
                v1.passes(summary["matched_minus_v5_calibrated_primary_target_margin"], THRESHOLDS["min_per_record_matched_minus_v5_calibrated_primary_target_margin"], ">"),
                f"> {THRESHOLDS['min_per_record_matched_minus_v5_calibrated_primary_target_margin']}",
                summary["matched_minus_v5_calibrated_primary_target_margin"],
            ),
            (
                "matched_minus_v5_calibrated_centroid_improvement",
                v1.passes(summary["matched_minus_v5_calibrated_centroid_improvement"], THRESHOLDS["min_per_record_matched_minus_v5_calibrated_centroid_improvement"], ">"),
                f"> {THRESHOLDS['min_per_record_matched_minus_v5_calibrated_centroid_improvement']}",
                summary["matched_minus_v5_calibrated_centroid_improvement"],
            ),
            (
                "source_primary_margin_change",
...
def gate_failures(
    *,
    ...
    aggregate_checks = [
        ...
        ("mean matched-minus-V5-calibrated primary target margin", aggregate["mean_matched_minus_v5_calibrated_primary_target_margin"], THRESHOLDS["min_mean_matched_minus_v5_calibrated_primary_target_margin"], ">"),
        ("mean matched-minus-V5-calibrated centroid improvement", aggregate["mean_matched_minus_v5_calibrated_centroid_improvement"], THRESHOLDS["min_mean_matched_minus_v5_calibrated_centroid_improvement"], ">"),
...
def record_passes(
    ...
    *,
    ...
    return (
        ...
        and summary["matched_minus_v5_calibrated_primary_target_margin"]
        > THRESHOLDS["min_per_record_matched_minus_v5_calibrated_primary_target_margin"]
        and summary["matched_minus_v5_calibrated_centroid_improvement"]
        > THRESHOLDS["min_per_record_matched_minus_v5_calibrated_centroid_improvement"]
...
def validate_source_pool_contract(
    *,
    ...
    failures = []
    if combined_audit.get("claim_scope") != "four_behavior_representation_steering_v6_source_pool_construction":
        failures.append("combined source-pool audit is not V6-specific")
    if final_redacted.get("claim_scope") != "redacted_final_steering_v6_source_pool_audit_surface_only":
        failures.append("final redacted audit is not V6-specific")
    if train_payload.get("claim_scope") != "four_behavior_representation_steering_v6_source_pool":
        failures.append("train source pool is not V6-specific")
    if eval_payload.get("claim_scope") != "four_behavior_representation_steering_v6_source_pool":
        failures.append(f"{phase} eval source pool is not V6-specific")
    ...
    if not combined_audit.get("seed_preflight", {}).get("passed", False):
        failures.append("V6 seed-range preflight did not pass")
