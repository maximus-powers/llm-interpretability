"""Four-behavior source-conditional tournament steering under V8 preregistration."""
...
DEFAULT_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_representation_steering_v8_pools"
TRAINING_CONFIG = {
...
V8_FINAL_RAW = DEFAULT_POOL_DIR / "final_subjects.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="runs/four_behavior_representation_steering_v8_source_conditional_tournament_correction",
    )
...
def validate_preregistered_args(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise ValueError("Non-preregistered V8 parameter override: " + "; ".join(failures))


def assert_preregistered_pool_dir(pool_dir: Path) -> None:
    resolved = pool_dir.resolve()
    if resolved != DEFAULT_POOL_DIR.resolve():
        raise ValueError(f"V8 requires preregistered pool directory: {DEFAULT_POOL_DIR}")
    for forbidden in (
        V1_POOL_DIR.resolve(),
        V2_POOL_DIR.resolve(),
        V3_POOL_DIR.resolve(),
        ...
        if resolved == forbidden or forbidden in resolved.parents:
            raise ValueError(f"V8 may not use prior steering source pools: {resolved}")
    ...
    *,
    ...
    for raw_path in paths:
        ...
        if path.name != "final_subjects.json":
            continue
        if allow_v8_final and path == V8_FINAL_RAW.resolve():
            continue
        raise ValueError(f"V8 steering final raw path is forbidden before final eval: {path}")
...
def generate_pools(args: argparse.Namespace, pool_dir: Path) -> Dict[str, Any]:
    ...
    for pool_name, pool_config in POOL_CONFIGS.items():
        ...
        payload["claim_scope"] = "four_behavior_representation_steering_v8_source_pool"
    ...
    final_redacted["claim_scope"] = "redacted_final_steering_v8_source_pool_audit_surface_only"
    ...
    audit["claim_scope"] = "four_behavior_representation_steering_v8_source_pool_construction"
    audit = redact_v8_combined_audit(audit)
...
def redact_v8_combined_audit(audit: Mapping[str, Any]) -> Dict[str, Any]:
...
def run_development(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths(paths.values(), allow_v8_final=False)
...
def run_final(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths([eval_path], allow_v8_final=True)
    payload = train_and_evaluate(
...
def train_and_evaluate(
    *,
    ...
    assert_no_forbidden_final_raw_paths(
        ...
        allow_v8_final=allow_eval_final_raw,
    )
    ...
    if hash_failures:
        raise ValueError("V8 source-pool file hash validation failed: " + "; ".join(hash_failures))
    ...
    if contract_failures:
        raise ValueError("V8 source-pool contract validation failed: " + "; ".join(contract_failures))
    ...
    transport_path = output_dir / "source_conditional_tournament_correction_stats.pt"
    torch.save(
        {
            ...
            "method": "train_source_conditional_tournament_correction",
    ...
    result = {
        **eval_result,
        ...
        "claim_scope": (
            ...
            else "four_behavior_representation_steering_v8_source_conditional_tournament_correction_final"
        ),
...
def fit_train_statistics(subjects: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    if vh.size(0) < rank:
        raise ValueError(f"V8 requires at least {rank} principal components")
    ...
    matched_candidates = [
        {
            ...
            "candidate_set_name": "matched_v8_source_conditional_tournament",
            "control_transport_key": v1.vector_key(source, target),
            "control_type": "matched_v8_source_conditional_tournament",
            **v1.score_candidate(
                z=z,
...
def build_control_candidate_specs(
    *,
    ...
    specs.extend(build_v7_frontier_candidate_specs(
        control_type="v7_pareto_frontier_correction",
        control_transport_key=v1.vector_key(source, target),
        candidate_set_name="v7_matched_pareto_frontier",
        z=z,
        source=source,
        target=target,
        train_stats=train_stats,
        classifier=classifier,
        metadata={"control_source_behavior": source, "control_target_behavior": target},
    ))
    specs.extend(build_v8_frontier_candidate_specs(
        control_type="reverse_v8_source_conditional_tournament_correction",
        control_transport_key=v1.vector_key(target, source),
        candidate_set_name="reverse_v8_source_conditional_tournament",
        z=z,
        source=target,
        target=source,
        train_stats=train_stats,
    ...
    for other_target in PATTERNS:
        if other_target in (source, target):
            continue
        specs.extend(build_v8_frontier_candidate_specs(
            control_type="same_source_other_target_v8_source_conditional_tournament_correction",
            control_transport_key=v1.vector_key(source, other_target),
            candidate_set_name="same_source_other_target_v8_source_conditional_tournament",
            z=z,
            source=source,
            target=other_target,
            train_stats=train_stats,
    ...
    for other_source in PATTERNS:
        if other_source in (source, target):
            continue
        specs.extend(build_v8_frontier_candidate_specs(
            control_type="same_target_other_source_v8_source_conditional_tournament_correction",
            control_transport_key=v1.vector_key(other_source, target),
            candidate_set_name="same_target_other_source_v8_source_conditional_tournament",
            z=z,
            source=other_source,
            target=target,
            train_stats=train_stats,
    ...
    specs.extend(build_v8_frontier_candidate_specs(
        control_type="shuffled_v8_source_conditional_tournament_correction",
        control_transport_key=v1.vector_key(shuffled_source, shuffled_target),
        candidate_set_name="shuffled_v8_source_conditional_tournament",
        z=z,
...
def select_shuffled_direction(
    ...
    digest = stable_hash_json([
        ...
        source,
        target,
        "representation_steering_v8_source_conditional_tournament_correction_shuffled_direction",
    ])
...
def random_norm_matched_vectors(
    *,
    ...
    seed_digest = stable_hash_json([
        ...
        source,
        target,
        "representation_steering_v8_source_conditional_tournament_correction_random",
    ])
...
def validate_source_pool_contract(
    *,
    ...
    failures = []
    if combined_audit.get("claim_scope") != "four_behavior_representation_steering_v8_source_pool_construction":
        failures.append("combined source-pool audit is not V8-specific")
    if final_redacted.get("claim_scope") != "redacted_final_steering_v8_source_pool_audit_surface_only":
        failures.append("final redacted audit is not V8-specific")
    if train_payload.get("claim_scope") != "four_behavior_representation_steering_v8_source_pool":
        failures.append("train source pool is not V8-specific")
    if eval_payload.get("claim_scope") != "four_behavior_representation_steering_v8_source_pool":
        failures.append(f"{phase} eval source pool is not V8-specific")
    ...
    if not combined_audit.get("seed_preflight", {}).get("passed", False):
        failures.append("V8 seed-range preflight did not pass")
