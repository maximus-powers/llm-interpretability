"""Four-behavior source-invariant target-attractor steering under V9 preregistration."""
...
DEFAULT_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_representation_steering_v9_pools"
TRAINING_CONFIG = {
    ...
    "same_target_centroid_tournament_loss_weight": 0.50,
    "same_target_primary_tournament_loss_weight": 0.20,
    "transport_method": "train_source_invariant_target_attractor",
    "v6_centroid_floor_margin": 0.06,
    "v6_centroid_floor_weight": 0.50,
...
THRESHOLDS = {
    ...
    "min_mean_same_target_transfer_gate_pass_rate": 0.80,
    "min_mean_same_target_transfer_target_prediction_rate": 0.90,
...
V7_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_representation_steering_v7_pools"
V8_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_representation_steering_v8_pools"
V9_FINAL_RAW = DEFAULT_POOL_DIR / "final_subjects.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="runs/four_behavior_representation_steering_v9_source_invariant_target_attractor",
    )
...
def validate_preregistered_args(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise ValueError("Non-preregistered V9 parameter override: " + "; ".join(failures))


def assert_preregistered_pool_dir(pool_dir: Path) -> None:
    resolved = pool_dir.resolve()
    if resolved != DEFAULT_POOL_DIR.resolve():
        raise ValueError(f"V9 requires preregistered pool directory: {DEFAULT_POOL_DIR}")
    for forbidden in (
        V1_POOL_DIR.resolve(),
        V2_POOL_DIR.resolve(),
        V3_POOL_DIR.resolve(),
        ...
        if resolved == forbidden or forbidden in resolved.parents:
            raise ValueError(f"V9 may not use prior steering source pools: {resolved}")
...
def assert_no_forbidden_final_raw_paths(
    ...
    *,
    ...
    forbidden = {
        ...
        (V7_POOL_DIR / "final_subjects.json").resolve(),
        (V8_POOL_DIR / "final_subjects.json").resolve(),
    }
    for raw_path in paths:
        ...
        if path.name != "final_subjects.json":
            continue
        if allow_v9_final and path == V9_FINAL_RAW.resolve():
            continue
        raise ValueError(f"V9 steering final raw path is forbidden before final eval: {path}")
...
def generate_pools(args: argparse.Namespace, pool_dir: Path) -> Dict[str, Any]:
    ...
    for pool_name, pool_config in POOL_CONFIGS.items():
        ...
        payload["claim_scope"] = "four_behavior_representation_steering_v9_source_pool"
    ...
    final_redacted["claim_scope"] = "redacted_final_steering_v9_source_pool_audit_surface_only"
    ...
    audit["claim_scope"] = "four_behavior_representation_steering_v9_source_pool_construction"
    audit = redact_v9_combined_audit(audit)
...
def redact_v9_combined_audit(audit: Mapping[str, Any]) -> Dict[str, Any]:
...
def run_development(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths(paths.values(), allow_v9_final=False)
...
def run_final(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths([eval_path], allow_v9_final=True)
    payload = train_and_evaluate(
...
def validate_development_authorizes_final(
    *,
    ...
    failures = []
    expected = {
        "claim_scope": "four_behavior_representation_steering_v9_source_invariant_target_attractor_development",
...
def train_and_evaluate(
    *,
    ...
    assert_no_forbidden_final_raw_paths(
        ...
        allow_v9_final=allow_eval_final_raw,
    )
    ...
    if hash_failures:
        raise ValueError("V9 source-pool file hash validation failed: " + "; ".join(hash_failures))
    ...
    if contract_failures:
        raise ValueError("V9 source-pool contract validation failed: " + "; ".join(contract_failures))
    ...
    transport_path = output_dir / "source_invariant_target_attractor_stats.pt"
    torch.save(
        {
            ...
            "method": "train_source_invariant_target_attractor",
    ...
    result = {
        **eval_result,
        ...
        "claim_scope": (
            ...
            else "four_behavior_representation_steering_v9_source_invariant_target_attractor_final"
        ),
        ...
        "forbidden_v8_steering_final_raw_opened": False,
        "forbidden_v9_steering_final_raw_opened": False,
        "phase": phase,
...
def fit_train_statistics(subjects: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    if vh.size(0) < rank:
        raise ValueError(f"V9 requires at least {rank} principal components")
...
def same_target_tournament_sources(*, source: str, target: str) -> list[str]:
    return [
        pattern for pattern in PATTERNS
        if pattern not in {source, target}
    ]


def build_same_target_tournament_competitors(
    *,
    z: torch.Tensor,
    source: str,
    target: str,
    ...
    for other_source in same_target_tournament_sources(source=source, target=target):
        competitors.extend(apply_v7_frontier(
            z=z,
            source=other_source,
            target=target,
            stats=stats,
    ...
    return [candidate.detach().clone() for candidate in competitors]


def apply_v8_frontier(
    *,
    z: torch.Tensor,
    source: str,
    target: str,
    ...
    tournament_competitors = build_same_target_tournament_competitors(
        z=z,
        source=source,
        target=target,
        stats=stats,
    ...
    return [
        apply_v8_radius_budget_correction(
            z=z,
            source=source,
            target=target,
            stats=stats,
...
def apply_v8_radius_budget_correction(
    *,
    z: torch.Tensor,
    source: str,
    target: str,
    ...
    v5: torch.Tensor,
    v6: torch.Tensor,
    tournament_competitors: Sequence[torch.Tensor],
    radius_budget: float,
) -> torch.Tensor:
    ...
    with torch.no_grad():
        v5_primary = primary_margin(classifier(v5), PATTERNS.index(target)).detach()
        v6_centroid = centroid_improvement(
            z=z,
            candidate_z=v6,
            target=target,
            stats=stats,
        ...
        competitor_scores = [
            (
                primary_margin(classifier(competitor), PATTERNS.index(target)).detach(),
                centroid_improvement(
                    z=z,
                    candidate_z=competitor,
                    target=target,
                    stats=stats,
                ).detach(),
            )
    ...
    for _ in range(int(TRAINING_CONFIG["correction_steps"])):
        ...
        candidate = forward_candidate()
        logits = classifier(candidate)
        candidate_primary = primary_margin(logits, PATTERNS.index(target))
        candidate_centroid = centroid_improvement(
            z=z,
        ...
        correction_norm_loss = TRAINING_CONFIG["coefficient_norm_loss_weight"] * q.pow(2).sum()
        v5_primary_floor_loss = 0.25 * F.relu(v5_primary - candidate_primary - 0.10)
        v6_centroid_floor_loss = (
            TRAINING_CONFIG["v6_centroid_floor_weight"]
            * F.relu(
                v6_centroid
                + TRAINING_CONFIG["v6_centroid_floor_margin"]
                - candidate_centroid
            )
        )
        competitor_primary_losses = [
            F.relu(competitor_primary - candidate_primary + 0.05)
            for competitor_primary, _ in competitor_scores
        ]
        competitor_centroid_losses = [
            F.relu(competitor_centroid - candidate_centroid + 0.05)
            for _, competitor_centroid in competitor_scores
        ]
        tournament_primary_loss = (
            TRAINING_CONFIG["same_target_primary_tournament_loss_weight"]
            * torch.stack(competitor_primary_losses).mean()
        )
        tournament_centroid_loss = (
            TRAINING_CONFIG["same_target_centroid_tournament_loss_weight"]
            * torch.stack(competitor_centroid_losses).mean()
        )
        loss = (
            target_ce
            + source_loss
            + correction_norm_loss
            + v5_primary_floor_loss
            + v6_centroid_floor_loss
            + tournament_primary_loss
            + tournament_centroid_loss
        )
...
def evaluate_record(
    *,
    ...
    matched_frontier = apply_v8_frontier(
        z=z,
        source=source,
        target=target,
        stats=train_stats,
    ...
    matched_candidates = [
        {
            ...
            "candidate_set_name": "matched_v9_source_invariant_target_attractor",
            "control_transport_key": v1.vector_key(source, target),
            "control_type": "matched_v9_source_invariant_target_attractor",
            **v1.score_candidate(
                z=z,
    ...
    specs = build_control_candidate_specs(
        classifier=classifier,
        subject_id=str(subject["subject_id"]),
        z=z,
        source=source,
    ...
    negative_specs, transfer_specs = split_negative_controls_and_transfer_probes(
        specs,
        source=source,
        target=target,
    )
    controls = score_candidate_specs(
    ...
    transfer_probes = score_candidate_specs(
    ...
    transfer_summary = summarize_transfer_probes(transfer_probes, target=target)
...
def is_transfer_probe_direction(
    *,
    matched_source: str,
    matched_target: str,
    control_source: str,
    ...
    return (
        control_target == matched_target
        and control_source not in {matched_source, matched_target}
    )


def split_negative_controls_and_transfer_probes(
    specs: Sequence[Mapping[str, Any]],
    *,
    source: str,
    target: str,
    ...
    for spec in specs:
        ...
        control_source = metadata.get("control_source_behavior")
        control_target = metadata.get("control_target_behavior")
        if (
            ...
            and is_transfer_probe_direction(
            ...
            transfer_probes.append(spec)
        else:
            negative_controls.append(spec)
    return negative_controls, transfer_probes


def summarize_transfer_probes(
    ...
    *,
    ...
    for probe in probes:
        predicts_target = (
            probe["primary_predicted_behavior"] == target
            and probe["centroid_predicted_behavior"] == target
        )
        if predicts_target:
            prediction_count += 1
        if (
            predicts_target
            and probe["primary_target_margin"] > THRESHOLDS["min_per_record_primary_target_margin"]
            and probe["centroid_improvement"] > THRESHOLDS["min_per_record_centroid_improvement"]
            and probe["primary_source_margin_change"]
            < THRESHOLDS["per_record_source_primary_margin_change_must_be_below"]
        ):
            gate_pass_count += 1
    probe_count = len(probes)
    return {
        "same_target_transfer_gate_pass_count": gate_pass_count,
        "same_target_transfer_gate_pass_rate": (
            gate_pass_count / probe_count if probe_count else 0.0
        ),
        "same_target_transfer_probe_count": probe_count,
        "same_target_transfer_target_prediction_count": prediction_count,
        "same_target_transfer_target_prediction_rate": (
            prediction_count / probe_count if probe_count else 0.0
        ),
    }
...
def build_control_candidate_specs(
    *,
    ...
    specs.extend(build_v8_frontier_candidate_specs(
        control_type="reverse_v9_source_invariant_target_attractor",
        control_transport_key=v1.vector_key(target, source),
        candidate_set_name="reverse_v9_source_invariant_target_attractor",
        z=z,
        source=target,
        target=source,
        train_stats=train_stats,
    ...
    for other_target in PATTERNS:
        ...
        specs.extend(build_v8_frontier_candidate_specs(
            ...
            candidate_set_name="same_source_other_target_v9_source_invariant_target_attractor",
            z=z,
            source=source,
            target=other_target,
            train_stats=train_stats,
    ...
    for other_source in PATTERNS:
        ...
        specs.extend(build_v8_frontier_candidate_specs(
            ...
            candidate_set_name="same_target_other_source_v9_source_invariant_target_attractor",
            z=z,
            source=other_source,
            target=target,
            train_stats=train_stats,
    ...
    specs.extend(build_v8_frontier_candidate_specs(
        ...
        candidate_set_name="shuffled_v9_source_invariant_target_attractor",
        z=z,
...
def build_v8_frontier_candidate_specs(
    *,
    control_type: str,
    control_transport_key: str,
    candidate_set_name: str,
...
def select_shuffled_direction(
    ...
    digest = stable_hash_json([
        ...
        source,
        target,
        "representation_steering_v9_source_invariant_target_attractor_shuffled_direction",
    ])
...
def random_norm_matched_vectors(
    *,
    ...
    seed_digest = stable_hash_json([
        ...
        source,
        target,
        "representation_steering_v9_source_invariant_target_attractor_random",
    ])
...
def summarize_records(records: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    same_target_transfer_probe_count = sum(
        int(item["same_target_transfer_probe_count"]) for item in summaries
    )
    same_target_transfer_target_prediction_count = sum(
        int(item["same_target_transfer_target_prediction_count"]) for item in summaries
    )
    same_target_transfer_gate_pass_count = sum(
        int(item["same_target_transfer_gate_pass_count"]) for item in summaries
    )
    return {
        ...
        "same_target_transfer_gate_pass_count": same_target_transfer_gate_pass_count,
        "same_target_transfer_gate_pass_rate": (
            same_target_transfer_gate_pass_count / same_target_transfer_probe_count
            if same_target_transfer_probe_count else 0.0
        ),
        "same_target_transfer_probe_count": same_target_transfer_probe_count,
        "same_target_transfer_target_prediction_count": (
            same_target_transfer_target_prediction_count
        ),
        "same_target_transfer_target_prediction_rate": (
            same_target_transfer_target_prediction_count / same_target_transfer_probe_count
            if same_target_transfer_probe_count else 0.0
        ),
...
def gate_failures(
    *,
    ...
    aggregate_checks = [
        ...
        ("same-target transfer target-prediction rate", aggregate["same_target_transfer_target_prediction_rate"], THRESHOLDS["min_mean_same_target_transfer_target_prediction_rate"], ">="),
        ("same-target transfer gate-pass rate", aggregate["same_target_transfer_gate_pass_rate"], THRESHOLDS["min_mean_same_target_transfer_gate_pass_rate"], ">="),
    ]
...
def validate_source_pool_contract(
    *,
    ...
    failures = []
    if combined_audit.get("claim_scope") != "four_behavior_representation_steering_v9_source_pool_construction":
        failures.append("combined source-pool audit is not V9-specific")
    if final_redacted.get("claim_scope") != "redacted_final_steering_v9_source_pool_audit_surface_only":
        failures.append("final redacted audit is not V9-specific")
    if train_payload.get("claim_scope") != "four_behavior_representation_steering_v9_source_pool":
        failures.append("train source pool is not V9-specific")
    if eval_payload.get("claim_scope") != "four_behavior_representation_steering_v9_source_pool":
        failures.append(f"{phase} eval source pool is not V9-specific")
    ...
    if not combined_audit.get("seed_preflight", {}).get("passed", False):
        failures.append("V9 seed-range preflight did not pass")
