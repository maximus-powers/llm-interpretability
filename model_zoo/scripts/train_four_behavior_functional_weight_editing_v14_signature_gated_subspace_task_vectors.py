"""V14 functional editing via signature-gated subspace task vectors."""
...
DEFAULT_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_functional_weight_editing_v14_pools"
DEFAULT_OUTPUT_DIR = (
    REPO_ROOT / "runs" / "four_behavior_functional_weight_editing_v14_signature_gated_subspace_task_vectors"
)
PREREG_PATH = (
    REPO_ROOT
    / "docs"
    / "preregistrations"
    / "four_behavior_functional_weight_editing_v14_signature_gated_subspace_task_vectors.md"
)
SCRIPT_PATH = Path(__file__).resolve()
SOURCE_POOL_SCOPE = "four_behavior_functional_weight_editing_v14_source_pool"
SOURCE_AUDIT_SCOPE = "four_behavior_functional_weight_editing_v14_source_pool_construction"
...
FINAL_SCOPE = "four_behavior_functional_weight_editing_v14_signature_gated_subspace_task_vectors_final"
EDITOR_METHOD = (
...
THRESHOLDS = {
    ...
    "min_aggregate_source_signature_target_margin_advantage": 0.02,
    "min_aggregate_source_signature_compatible_mse_advantage": 2.0,
    "min_aggregate_target_label_target_margin_advantage": 0.02,
    "min_aggregate_target_label_compatible_mse_advantage": 2.0,
    "min_aggregate_v13_no_signature_target_margin_advantage": 0.02,
    "min_aggregate_v13_no_signature_compatible_mse_advantage": 2.0,
    ...
    "min_per_record_source_signature_target_margin_advantage": 0.02,
    "min_per_record_source_signature_compatible_mse_advantage": 5.0,
    "min_per_record_target_label_target_margin_advantage": 0.02,
    "min_per_record_target_label_compatible_mse_advantage": 5.0,
...
V14_FINAL_RAW = DEFAULT_POOL_DIR / "final_subjects.json"
...
def validate_preregistered_args(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise ValueError("Non-preregistered V14 parameter override: " + "; ".join(failures))


def assert_preregistered_pool_dir(pool_dir: Path) -> None:
    resolved = pool_dir.resolve()
    if resolved != DEFAULT_POOL_DIR.resolve():
        raise ValueError(f"V14 requires preregistered pool directory: {DEFAULT_POOL_DIR}")


def assert_no_forbidden_final_raw_paths(
    paths: Iterable[Path | str],
    *,
    allow_v14_final: bool,
) -> None:
    ...
    for raw_path in paths:
        ...
        if path.name != "final_subjects.json":
            continue
        if allow_v14_final and path == V14_FINAL_RAW.resolve():
            continue
        raise ValueError(f"V14 final raw path is forbidden before final eval: {path}")
...
def run_development(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths(paths.values(), allow_v14_final=False)
...
def run_final(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths([eval_path], allow_v14_final=True)
    payload = train_and_evaluate(
...
def train_and_evaluate(
    *,
    ...
    assert_no_forbidden_final_raw_paths(
        ...
        allow_v14_final=allow_eval_final_raw,
    )
    ...
    if contract_failures:
        raise ValueError("V14 source-pool contract validation failed: " + "; ".join(contract_failures))

    train_subjects = v1.accepted_records(train_payload)
    eval_subjects = v1.accepted_records(eval_payload)
    train_stats = fit_v14_train_statistics(train_subjects)
    probe_examples = build_digit_probe_examples(
        n_examples=256,
        seed=20260610,
        seq_len=5,
    ...
    stats_path = output_dir / "v14_signature_gated_subspace_task_vectors_stats.pt"
    torch.save(
        {
            "calibration_coefficients": train_stats["calibration_coefficients"],
            "centroids": train_stats["centroids"],
            "edit_subspace_diagnostics": edit_subspace_diagnostics(train_stats),
    ...
    result = {
        **eval_result,
        ...
        "forbidden_v14_final_raw_opened_before_authorization": False,
        "implementation_sha256": v1.sha256_file(SCRIPT_PATH),
        "limitations": (
            "Small-subject source-label-known, target-label-requested signature-gated "
            "low-rank task-vector evidence only; not pure signature-only editing, "
            "source-label inference, source-free decoding, larger-model evidence, "
            "broad MUAT proof, or arbitrary capability preservation."
        ),
        "subspace_config": {
            "rank": SUBSPACE_RANK,
...
def validate_source_pool_contract(
    *,
    ...
    if phase == "development":
        assert_no_forbidden_final_raw_paths(
            [train_path, eval_path, combined_audit_path, final_redacted_path],
            allow_v14_final=False,
        )
    return failures


def fit_v14_train_statistics(subjects: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    stats["train_by_behavior"] = {
        pattern: [record for record in subjects if v1.behavior_of(record) == pattern]
        for pattern in PATTERNS
    }
    ...
    stats["edit_subspaces"] = build_edit_subspaces(stats["train_by_behavior"])
    return stats


def build_edit_subspaces(
    train_by_behavior: Mapping[str, Sequence[Mapping[str, Any]]],
) -> Dict[str, Dict[str, Any]]:
    subspaces: Dict[str, Dict[str, Any]] = {}
    for source in PATTERNS:
        source_subjects = list(train_by_behavior[source])
        for target in PATTERNS:
            if target == source:
                continue
            target_subjects = list(train_by_behavior[target])
            deltas = []
            for source_subject in source_subjects:
                source_weights = torch.tensor(source_subject["weights"], dtype=torch.float32)
                for target_subject in target_subjects:
                    target_weights = torch.tensor(target_subject["weights"], dtype=torch.float32)
                    aligned = align_target_to_source(
                        source_weights=source_weights,
                        target_weights=target_weights,
                    )
                    deltas.append(aligned - source_weights)
            delta_matrix = torch.stack(deltas).float()
            mean_delta = delta_matrix.mean(dim=0)
            centered = delta_matrix - mean_delta
            _, singular_values, vh = torch.linalg.svd(centered, full_matrices=False)
            basis = vh[:SUBSPACE_RANK].contiguous()
            total_variance = singular_values.pow(2).sum().clamp_min(1e-12)
            explained = singular_values[:SUBSPACE_RANK].pow(2).sum() / total_variance
            direction = v1.vector_key(source, target)
            subspaces[direction] = {
                "basis": basis,
                "explained_variance": float(explained.item()),
                "mean_delta": mean_delta,
                "mean_delta_norm": float(mean_delta.norm().item()),
                "rank": SUBSPACE_RANK,
...
def edit_subspace(
    ...
    return train_stats["edit_subspaces"][v1.vector_key(source, target)]


def project_delta_to_subspace(
    delta: torch.Tensor,
    subspace: Mapping[str, Any],
) -> torch.Tensor:
    mean_delta = subspace["mean_delta"]
    basis = subspace["basis"]
    centered = delta - mean_delta
    return mean_delta + torch.matmul(torch.matmul(centered, basis.t()), basis)


def normalized_record_signature(
    record: Mapping[str, Any],
    train_stats: Mapping[str, Any],
) -> torch.Tensor:
    signature = torch.tensor(record["signature"], dtype=torch.float32)
    return (signature - train_stats["sig_mean"]) / train_stats["sig_std"]


def signature_target_weights(
    ...
    *,
    ...
    for record in target_records:
        z_record = normalized_record_signature(record, train_stats)
        distance = float(F.mse_loss(z_record, signature_target_norm).item())
        scored.append((distance, str(record["subject_id"]), record, z_record))
...
def random_permute_target_weights(
    *,
    ...
    layers = unpack_subject_weights(target_weights)
    ...
    for layer_index in range(5):
        ...
        weights[layer_index] = weights[layer_index][permutation]
        biases[layer_index] = biases[layer_index][permutation]
        weights[layer_index + 1] = weights[layer_index + 1][:, permutation]
    return pack_subject_weights(weights, biases)


def target_delta_for_record(
    *,
    ...
    if alignment_mode == "hungarian":
        target_weights = align_target_to_source(
            source_weights=source_weights,
            target_weights=target_weights,
        )
    elif alignment_mode == "random_neuron_permutation":
        target_weights = random_permute_target_weights(
            target_weights=target_weights,
...
def signature_weighted_task_direction(
    *,
    subject_id: str,
    source_weights: torch.Tensor,
    source: str,
    ...
    target_records = train_target_records(train_stats, target)
    weights_info = signature_target_weights(
        target_records,
    ...
    subspace = edit_subspace(train_stats, source, target)
    unprojected = torch.zeros_like(source_weights)
    projected = torch.zeros_like(source_weights)
    selected_metadata = []
    for index, (distance, target_subject_id, record, _z_record) in enumerate(
        weights_info["selected"]
    ):
        raw_delta = target_delta_for_record(
            source_weights=source_weights,
        ...
        projected_delta = project_delta_to_subspace(raw_delta, subspace)
        weight = weights_info["softmax_weights"][index]
        unprojected = unprojected + weight * raw_delta
        projected = projected + weight * projected_delta
        selected_metadata.append({
            "distance": float(distance),
            "softmax_weight": float(weight.item()),
            "target_subject_id": target_subject_id,
        })
    return projected, unprojected, {
        "alignment_mode": alignment_mode,
        "selected_target_subjects": selected_metadata,
        "signature_temperature": SIGNATURE_TEMPERATURE,
        "top_k": int(weights_info["top_k"]),
    }


def uniform_average_task_direction(
    *,
    subject_id: str,
    source_weights: torch.Tensor,
    source: str,
    ...
    subspace = edit_subspace(train_stats, source, target)
    projected_deltas = []
    for record in train_target_records(train_stats, target):
        raw_delta = target_delta_for_record(
            source_weights=source_weights,
            target_record=record,
            source=source,
        ...
        projected_deltas.append(project_delta_to_subspace(raw_delta, subspace))
    direction = torch.stack(projected_deltas).mean(dim=0)
    return direction, {"projected_delta_count": len(projected_deltas)}


def ties_trimmed_sign_task_direction(
    *,
    subject_id: str,
    source_weights: torch.Tensor,
    source: str,
    ...
    for record in train_target_records(train_stats, target):
        ...
        projected_deltas.append(project_delta_to_subspace(raw_delta, subspace))
    matrix = torch.stack(projected_deltas)
    thresholds = torch.quantile(matrix.abs(), 0.20, dim=0)
    keep = matrix.abs() >= thresholds
    trimmed = torch.where(keep, matrix, torch.zeros_like(matrix))
    elected_sign = torch.sign(trimmed.sum(dim=0))
    elected = keep & (torch.sign(matrix) == elected_sign.unsqueeze(0)) & (
        elected_sign.unsqueeze(0) != 0
    )
    counts = elected.sum(dim=0)
    summed = torch.where(elected, matrix, torch.zeros_like(matrix)).sum(dim=0)
    direction = torch.where(counts > 0, summed / counts.clamp_min(1), torch.zeros_like(summed))
    return direction, {
        "candidate_delta_count": int(matrix.shape[0]),
        "mean_elected_coordinate_count": float(counts.float().mean().item()),
        "trim_percentile": 0.20,
    }


def random_same_rank_subspace_direction(
    *,
    subject_id: str,
    source: str,
    target: str,
    unprojected_direction: torch.Tensor,
) -> tuple[torch.Tensor, Dict[str, Any]]:
    key = stable_hash_json([
        subject_id,
        source,
    ...
    generator = torch.Generator().manual_seed(seed)
    matrix = torch.randn(
        unprojected_direction.numel(),
        SUBSPACE_RANK,
        generator=generator,
        dtype=unprojected_direction.dtype,
    )
    q, _ = torch.linalg.qr(matrix, mode="reduced")
    direction = torch.matmul(q, torch.matmul(q.t(), unprojected_direction))
    return direction, {"random_seed": int(seed), "rank": SUBSPACE_RANK}
...
def evaluate_record(
    *,
    ...
    matched_direction, matched_unprojected_direction, matched_direction_meta = (
        signature_weighted_task_direction(
    ...
    matched_weights, matched_scale_meta = select_scaled_task_vector_weights(
        source_weights=source_weights,
        source=source,
        target=target,
    ...
    matched = {
        **functional_metrics(matched_weights, source, target, source_weights),
        ...
        "direction_norm": float(matched_direction.norm().item()),
        "editor_scale": matched_scale_meta["selected_scale"],
        "scale_selection": matched_scale_meta,
        ...
        "signature_weighting": matched_direction_meta,
        "subspace_explained_variance": edit_subspace(
            train_stats,
            source,
            target,
        )["explained_variance"],
    ...
    controls = build_controls(
        ...
        matched_unprojected_direction=matched_unprojected_direction,
        train_stats=train_stats,
        classifier=classifier,
        random_controls=random_controls,
    )
    ...
    target_label = single_control(controls, "target_label_centroid_task_vector")
    source_signature = single_control(controls, "source_signature_weighted_task_vector")
    shuffled_signature = single_control(controls, "shuffled_signature_weighted_task_vector")
    v13_no_signature = single_control(controls, "v13_no_signature_support_optimizer")
    matched["matched_minus_target_label_target_margin"] = (
        matched["target_margin"] - target_label["target_margin"]
    )
    matched["target_label_minus_matched_compatible_source_output_mse"] = (
        target_label["compatible_source_output_mse"]
        - matched["compatible_source_output_mse"]
    )
    matched["matched_minus_source_signature_target_margin"] = (
        matched["target_margin"] - source_signature["target_margin"]
    )
    matched["source_signature_minus_matched_compatible_source_output_mse"] = (
        source_signature["compatible_source_output_mse"]
        - matched["compatible_source_output_mse"]
    )
    ...
    matched["matched_minus_v13_no_signature_target_margin"] = (
        matched["target_margin"] - v13_no_signature["target_margin"]
    )
    matched["v13_no_signature_minus_matched_compatible_source_output_mse"] = (
        v13_no_signature["compatible_source_output_mse"]
        - matched["compatible_source_output_mse"]
    )
    ...
    return {
        ...
        "summary": {
            ...
            "matched_minus_source_signature_target_margin": matched[
                "matched_minus_source_signature_target_margin"
            ],
            "matched_minus_target_label_target_margin": matched[
                "matched_minus_target_label_target_margin"
            ],
            ...
            "source_signature_minus_matched_compatible_source_output_mse": matched[
                "source_signature_minus_matched_compatible_source_output_mse"
            ],
            "target_prediction_pass": matched["target_prediction_pass"],
            "target_label_minus_matched_compatible_source_output_mse": matched[
                "target_label_minus_matched_compatible_source_output_mse"
            ],
            "matched_minus_v13_no_signature_target_margin": matched[
                "matched_minus_v13_no_signature_target_margin"
            ],
            "v13_no_signature_minus_matched_compatible_source_output_mse": matched[
                "v13_no_signature_minus_matched_compatible_source_output_mse"
            ],
        },
...
def build_controls(
    *,
    ...
    matched_unprojected_direction: torch.Tensor,
    train_stats: Mapping[str, Any],
    classifier: torch.nn.Module,
    random_controls: int,
) -> list[Dict[str, Any]]:
    ...
    subspace = edit_subspace(train_stats, source, target)
    controls.append(task_vector_control_record(
        control_type="target_label_centroid_task_vector",
        direction=subspace["mean_delta"],
        source_weights=source_weights,
        source=source,
        target=target,
        metadata={
            "mean_delta_norm": float(subspace["mean_delta"].norm().item()),
            "subspace_rank": SUBSPACE_RANK,
        },
    ))
    nearest_direction, _nearest_unprojected, nearest_meta = signature_weighted_task_direction(
        subject_id=str(subject["subject_id"]),
        source_weights=source_weights,
        source=source,
        target=target,
    ...
    uniform_direction, uniform_meta = uniform_average_task_direction(
        subject_id=str(subject["subject_id"]),
        source_weights=source_weights,
        source=source,
        target=target,
    ...
    controls.append(task_vector_control_record(
        ...
        direction=uniform_direction,
        source_weights=source_weights,
        source=source,
        target=target,
        metadata=uniform_meta,
    ))
    ...
    shuffled_direction, _shuffled_unprojected, shuffled_meta = signature_weighted_task_direction(
        subject_id=str(subject["subject_id"]),
        source_weights=source_weights,
        source=source,
        target=target,
    ...
    controls.append(task_vector_control_record(
        ...
        source=source,
        target=target,
        metadata={**shuffled_meta, "shuffled_target_behavior": shuffled},
    ))
    source_signature_direction, _source_unprojected, source_signature_meta = (
        signature_weighted_task_direction(
            subject_id=str(subject["subject_id"]),
            source_weights=source_weights,
            source=source,
    ...
    controls.append(task_vector_control_record(
        ...
        direction=source_signature_direction,
        source_weights=source_weights,
        source=source,
        target=target,
        metadata=source_signature_meta,
    ))
    ties_direction, ties_meta = ties_trimmed_sign_task_direction(
        subject_id=str(subject["subject_id"]),
        source_weights=source_weights,
        source=source,
        target=target,
    ...
    controls.append(task_vector_control_record(
        ...
        direction=ties_direction,
        source_weights=source_weights,
        source=source,
        target=target,
        metadata=ties_meta,
    ))
    repair_weights, repair_meta = repair_style_aligned_interpolation_weights(
        source_weights=source_weights,
        aligned_target_weights=retrieved_weights,
        source=source,
        target=target,
    )
    controls.append(control_record(
        "repair_style_aligned_interpolation",
        repair_weights,
        source,
        target,
        source_weights,
        {"alpha": MATCHED_ALPHA, "retrieved_subject_id": retrieved_subject_id, **repair_meta},
    ))
    random_subspace_direction, random_subspace_meta = random_same_rank_subspace_direction(
        subject_id=str(subject["subject_id"]),
        source=source,
        target=target,
        unprojected_direction=matched_unprojected_direction,
    )
    controls.append(task_vector_control_record(
        control_type="random_same_rank_subspace_task_vector",
        direction=random_subspace_direction,
        source_weights=source_weights,
        source=source,
        target=target,
        metadata=random_subspace_meta,
    ))
    random_perm_direction, _random_perm_unprojected, random_perm_meta = (
        signature_weighted_task_direction(
            subject_id=str(subject["subject_id"]),
            source_weights=source_weights,
            source=source,
    ...
    controls.append(task_vector_control_record(
        ...
        direction=random_perm_direction,
        source_weights=source_weights,
        source=source,
        target=target,
        metadata=random_perm_meta,
    ))
    no_alignment_direction, _no_alignment_unprojected, no_alignment_meta = (
        signature_weighted_task_direction(
            subject_id=str(subject["subject_id"]),
            source_weights=source_weights,
            source=source,
    ...
    controls.append(task_vector_control_record(
        ...
        direction=no_alignment_direction,
        source_weights=source_weights,
        source=source,
        target=target,
        metadata=no_alignment_meta,
    ))
...
def select_scaled_task_vector_weights(
    *,
    source_weights: torch.Tensor,
    source: str,
    target: str,
    ...
    for scale in SCALE_GRID:
        weights = source_weights + float(scale) * direction
        losses = support_scale_objective(
            weights=weights,
            source_weights=source_weights,
            source=source,
            target=target,
    ...
    best = min(
        candidates,
        key=lambda item: (item["objective"], abs(item["scale"]), item["scale"]),
    )
    selected_weights = source_weights + best["scale"] * direction
    return selected_weights, {
        "candidate_objectives": candidates,
        "direction_norm": float(direction.norm().item()),
        "scale_grid": [float(scale) for scale in SCALE_GRID],
        "selected_objective": float(best["objective"]),
        "selected_scale": float(best["scale"]),
    }


def support_scale_objective(
    *,
    ...
    with torch.no_grad():
        target_logits = v10.decoder_v1.subject_forward_flat_batch(
            weights.unsqueeze(0),
            support["target_inputs"],
        )[0]
        conflict_logits = v10.decoder_v1.subject_forward_flat_batch(
            weights.unsqueeze(0),
            support["conflict_inputs"],
        )[0]
        compatible_logits = v10.decoder_v1.subject_forward_flat_batch(
            weights.unsqueeze(0),
        ...
        source_l2 = F.mse_loss(weights, source_weights)
        objective = (
            TARGET_SUPPORT_BCE_WEIGHT * target_bce
            + CONFLICT_SUPPORT_BCE_WEIGHT * conflict_bce
            + COMPATIBLE_SUPPORT_MSE_WEIGHT * compatible_mse
            + SOURCE_WEIGHT_L2_WEIGHT * source_l2
        )
    return {
        "compatible_mse": float(compatible_mse.item()),
        "conflict_bce": float(conflict_bce.item()),
        "l2_mse": float(source_l2.item()),
        "objective": float(objective.item()),
        "target_bce": float(target_bce.item()),
    }


def task_vector_control_record(
    *,
    control_type: str,
    direction: torch.Tensor,
    source_weights: torch.Tensor,
    ...
    weights, scale_meta = select_scaled_task_vector_weights(
        source_weights=source_weights,
        source=source,
        target=target,
        direction=direction,
...
def support_union_inputs(*, source: str, target: str) -> torch.Tensor:
    suite = v10.evaluation_suite()
    sequences = {
        tuple(sequence)
        for behavior in (source, target)
        for polarity in ("positive", "negative")
        for sequence in suite["support"][behavior][polarity]
    }
    return v10.decoder_v1.sequence_tensor(sorted(sequences))


def repair_style_aligned_interpolation_weights(
    *,
    source_weights: torch.Tensor,
    ...
    interpolation = interpolate_weights(
        source_weights=source_weights,
        target_weights=aligned_target_weights,
        alpha=MATCHED_ALPHA,
    )
    inputs = support_union_inputs(source=source, target=target)
    source_activations = hidden_activations_flat(source_weights, inputs)
    target_activations = hidden_activations_flat(aligned_target_weights, inputs)
    interpolation_activations = hidden_activations_flat(interpolation, inputs)
    repaired = unpack_subject_weights(interpolation)
    weights = repaired["weights"]
    biases = repaired["biases"]
    layer_diagnostics = []
    for layer_index in range(5):
        source_mean = source_activations[layer_index].mean(dim=0)
        target_mean = target_activations[layer_index].mean(dim=0)
        interpolation_mean = interpolation_activations[layer_index].mean(dim=0)
        source_std = source_activations[layer_index].std(dim=0, unbiased=False)
        target_std = target_activations[layer_index].std(dim=0, unbiased=False)
        interpolation_std = interpolation_activations[layer_index].std(dim=0, unbiased=False)
        desired_mean = 0.5 * (source_mean + target_mean)
        desired_std = 0.5 * (source_std + target_std)
        repair_ratio = (desired_std / interpolation_std.clamp_min(1e-6)).clamp(0.25, 4.0)
        next_weight_before_scaling = weights[layer_index + 1].clone()
        bias_adjustment = torch.matmul(
            next_weight_before_scaling,
            desired_mean - repair_ratio * interpolation_mean,
        )
        weights[layer_index + 1] = weights[layer_index + 1] * repair_ratio.unsqueeze(0)
        biases[layer_index + 1] = biases[layer_index + 1] + bias_adjustment
        layer_diagnostics.append({
            "desired_std_mean": float(desired_std.mean().item()),
            "interpolation_std_mean": float(interpolation_std.mean().item()),
            "layer_index": int(layer_index),
            "repair_ratio_mean": float(repair_ratio.mean().item()),
            "source_std_mean": float(source_std.mean().item()),
            "target_std_mean": float(target_std.mean().item()),
        })
    return pack_subject_weights(weights, biases), {
        "alpha": MATCHED_ALPHA,
        "repair_layers": layer_diagnostics,
        "support_union_count": int(inputs.shape[0]),
    }
...
def summarize_records(records: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "mean_matched_minus_source_signature_target_margin": v10.mean(
            record["summary"]["matched_minus_source_signature_target_margin"]
            for record in records
        ),
        "mean_matched_minus_target_label_target_margin": v10.mean(
            record["summary"]["matched_minus_target_label_target_margin"] for record in records
        ),
        ...
        "mean_matched_minus_v13_no_signature_target_margin": v10.mean(
            record["summary"]["matched_minus_v13_no_signature_target_margin"]
            for record in records
        ),
        "mean_matched_target_margin": v10.mean(
            record["matched"]["target_margin"] for record in records
        ),
        "mean_source_signature_minus_matched_compatible_source_output_mse": v10.mean(
            record["summary"]["source_signature_minus_matched_compatible_source_output_mse"]
            for record in records
        ),
        "mean_target_label_minus_matched_compatible_source_output_mse": v10.mean(
            record["summary"]["target_label_minus_matched_compatible_source_output_mse"]
            for record in records
        ),
        ...
        "mean_v13_no_signature_minus_matched_compatible_source_output_mse": v10.mean(
            record["summary"]["v13_no_signature_minus_matched_compatible_source_output_mse"]
            for record in records
        ),
        "n": int(n),
...
def gate_failures(
    *,
    ...
    if not (
        aggregate["mean_matched_minus_target_label_target_margin"]
        > THRESHOLDS["min_aggregate_target_label_target_margin_advantage"]
        or aggregate["mean_target_label_minus_matched_compatible_source_output_mse"]
        > THRESHOLDS["min_aggregate_target_label_compatible_mse_advantage"]
    ):
        failures.append("aggregate target-label advantage gate failed")
    if not (
        aggregate["mean_matched_minus_source_signature_target_margin"]
        > THRESHOLDS["min_aggregate_source_signature_target_margin_advantage"]
        or aggregate["mean_source_signature_minus_matched_compatible_source_output_mse"]
        > THRESHOLDS["min_aggregate_source_signature_compatible_mse_advantage"]
    ):
        failures.append("aggregate source-signature advantage gate failed")
    if not (
    ...
    if not (
        aggregate["mean_matched_minus_v13_no_signature_target_margin"]
        > THRESHOLDS["min_aggregate_v13_no_signature_target_margin_advantage"]
        or aggregate["mean_v13_no_signature_minus_matched_compatible_source_output_mse"]
        > THRESHOLDS["min_aggregate_v13_no_signature_compatible_mse_advantage"]
    ):
        failures.append("aggregate v13-no-signature advantage gate failed")
    ...
    for record in records:
        ...
        required_control_types = {
            ...
            "aligned_interpolation_alpha_0.975",
            "no_edit",
            "nearest_signature_task_vector",
            "no_alignment_task_vector",
            "random_neuron_permutation_task_vector",
            "random_same_rank_subspace_task_vector",
            "repair_style_aligned_interpolation",
            "shuffled_signature_weighted_task_vector",
            "source_signature_weighted_task_vector",
            "target_label_centroid_task_vector",
            "ties_trimmed_sign_task_vector",
            "uniform_average_task_vector",
...
def train_only_statistics_hash(stats: Mapping[str, Any]) -> str:
    return stable_hash_json({
        ...
        "edit_subspaces": {
            direction: {
                "basis": v10.tensor_to_hashable(subspace["basis"]),
                "explained_variance": subspace["explained_variance"],
                "mean_delta": v10.tensor_to_hashable(subspace["mean_delta"]),
                "rank": subspace["rank"],
            }
            for direction, subspace in sorted(stats["edit_subspaces"].items())
        },
...
def edit_subspace_diagnostics(stats: Mapping[str, Any]) -> Dict[str, Any]:
    return {
        direction: {
            "explained_variance": subspace["explained_variance"],
            "mean_delta_norm": subspace["mean_delta_norm"],
            "rank": subspace["rank"],
            "singular_values_head": [
                float(value)
                for value in subspace["singular_values"][: min(8, len(subspace["singular_values"]))]
            ],
        }
        for direction, subspace in sorted(stats["edit_subspaces"].items())
    }
