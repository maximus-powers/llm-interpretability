"""V11 source-label-known functional editing via retrieval interpolation."""
...
DEFAULT_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_functional_weight_editing_v11_pools"
DEFAULT_OUTPUT_DIR = (
    REPO_ROOT / "runs" / "four_behavior_functional_weight_editing_v11_retrieval_interpolation"
)
PREREG_PATH = (
    REPO_ROOT
    / "docs"
    / "preregistrations"
    / "four_behavior_functional_weight_editing_v11_retrieval_interpolation.md"
)
SCRIPT_PATH = Path(__file__).resolve()
SOURCE_POOL_SCOPE = "four_behavior_functional_weight_editing_v11_source_pool"
SOURCE_AUDIT_SCOPE = "four_behavior_functional_weight_editing_v11_source_pool_construction"
...
FINAL_SCOPE = "four_behavior_functional_weight_editing_v11_retrieval_interpolation_final"
EDITOR_METHOD = "v9_selected_target_retrieval_raw_weight_interpolation_alpha_0_95"
MATCHED_ALPHA = 0.95
CONTROL_ALPHAS = [0.25, 0.50, 0.75, 0.85, 0.90]
...
THRESHOLDS = {
    "expected_controls_per_record": 44,
...
V11_FINAL_RAW = DEFAULT_POOL_DIR / "final_subjects.json"
...
def validate_preregistered_args(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise ValueError("Non-preregistered V11 parameter override: " + "; ".join(failures))


def assert_preregistered_pool_dir(pool_dir: Path) -> None:
    resolved = pool_dir.resolve()
    if resolved != DEFAULT_POOL_DIR.resolve():
        raise ValueError(f"V11 requires preregistered pool directory: {DEFAULT_POOL_DIR}")


def assert_no_forbidden_final_raw_paths(
    paths: Iterable[Path | str],
    *,
    allow_v11_final: bool,
) -> None:
    ...
    for raw_path in paths:
        ...
        if path.name != "final_subjects.json":
            continue
        if allow_v11_final and path == V11_FINAL_RAW.resolve():
            continue
        raise ValueError(f"V11 final raw path is forbidden before final eval: {path}")
...
def run_development(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths(paths.values(), allow_v11_final=False)
...
def run_final(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths([eval_path], allow_v11_final=True)
    payload = train_and_evaluate(
...
def train_and_evaluate(
    *,
    ...
    assert_no_forbidden_final_raw_paths(
        ...
        allow_v11_final=allow_eval_final_raw,
    )
    ...
    if contract_failures:
        raise ValueError("V11 source-pool contract validation failed: " + "; ".join(contract_failures))

    train_subjects = v1.accepted_records(train_payload)
    eval_subjects = v1.accepted_records(eval_payload)
    train_stats = fit_v11_train_statistics(train_subjects)
    ...
    stats_path = output_dir / "v11_retrieval_interpolation_stats.pt"
    torch.save(
        {
            "alpha": MATCHED_ALPHA,
    ...
    result = {
        **eval_result,
        ...
        "forbidden_v11_final_raw_opened_before_authorization": False,
        "implementation_sha256": v1.sha256_file(SCRIPT_PATH),
        "limitations": (
...
def validate_source_pool_contract(
    *,
    ...
    if phase == "development":
        assert_no_forbidden_final_raw_paths(
            [train_path, eval_path, combined_audit_path, final_redacted_path],
            allow_v11_final=False,
        )
    return failures


def fit_v11_train_statistics(subjects: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
...
def evaluate_record(
    *,
    ...
    retrieved_weights = retrieval["weights"]
    ...
    matched = {
        **v10.functional_metrics(matched_weights, source, target, source_weights),
        "alpha": MATCHED_ALPHA,
    ...
    full_retrieval = next(control for control in controls if control["control_type"] == "full_nearest_target_retrieval")
    best_target = max(controls, key=lambda item: item["target_margin"])
    pareto_dominators = [
        control for control in controls if v10.pareto_dominates_functional(control, matched)
    ]
    ...
    return {
        ...
        "summary": {
            ...
            "full_retrieval_minus_matched_source_output_mse": (
                full_retrieval["source_output_mse"] - matched["source_output_mse"]
            ),
...
def build_controls(
    *,
    ...
    controls = []
    controls.append(v10.control_record("no_edit", source_weights, source, target, source_weights))
    controls.append(v10.control_record(
        "full_nearest_target_retrieval",
        retrieved_weights,
        source,
        target,
        source_weights,
...
def select_shuffled_target(*, subject_id: str, source: str, target: str) -> str:
    ...
    key = stable_hash_json([
        ...
        source,
        target,
        "functional_weight_editing_v11_shuffled_target",
    ])
...
def random_weight_delta_controls(
    *,
    ...
    key = stable_hash_json([
        ...
        source,
        target,
        "functional_weight_editing_v11_random_weight_delta",
    ])
...
def individual_passed(
    *,
    ...
    return (
        ...
        and matched["source_output_mse"] < full_retrieval["source_output_mse"]
...
def summarize_records(records: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "mean_full_retrieval_minus_matched_source_output_mse": v10.mean(
            record["summary"]["full_retrieval_minus_matched_source_output_mse"]
        ...
        "mean_matched_target_vs_source_margin": v10.mean(
...
def gate_failures(
    *,
    ...
    v10.require_greater(
        failures,
        aggregate["mean_matched_minus_full_retrieval_target_margin"],
        THRESHOLDS["min_mean_matched_minus_full_retrieval_target_margin"],
        "mean matched-minus-full-retrieval target margin",
    )
    v10.require_greater(
        failures,
        aggregate["mean_full_retrieval_minus_matched_source_output_mse"],
        THRESHOLDS["min_mean_full_retrieval_minus_matched_source_output_mse"],
        "mean full-retrieval-minus-matched source-output MSE",
    )
    for direction, summary in by_direction.items():
        ...
        v10.require_greater(
            failures,
            summary["mean_full_retrieval_minus_matched_source_output_mse"],
            THRESHOLDS["min_direction_mean_full_retrieval_minus_matched_source_output_mse"],
            f"{direction} full-retrieval-minus-matched source-output MSE",
        )
        v10.require_at_least(
            failures,
    ...
    for record in records:
        ...
        if sum(
            ...
            if control["control_type"] == "full_nearest_target_retrieval"
        ) != 1:
            failures.append(f"{record['subject_id']} full retrieval control count mismatch")
        if not any(
            control["control_type"] == "full_nearest_target_retrieval"
            for control in record["controls"]
        ):
            failures.append(f"{record['subject_id']} missing full retrieval control")
    return failures
