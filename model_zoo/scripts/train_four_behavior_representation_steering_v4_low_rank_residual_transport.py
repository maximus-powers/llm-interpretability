"""Four-behavior low-rank residual representation steering under V4 preregistration."""
...
DEFAULT_POOL_DIR = REPO_ROOT / "runs" / "four_behavior_representation_steering_v4_pools"
TRAINING_CONFIG = {
    "classifier_epochs": 1000,
    "classifier_lr": 0.10,
    "classifier_seed": 20260711,
    ...
    "pca_rank": 48,
    "transport_method": "train_low_rank_residual_covariance_transport_no_optimizer",
...
V4_FINAL_RAW = DEFAULT_POOL_DIR / "final_subjects.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default="runs/four_behavior_representation_steering_v4_low_rank_residual_transport",
    )
...
def validate_preregistered_args(args: argparse.Namespace) -> None:
    ...
    if failures:
        raise ValueError("Non-preregistered V4 parameter override: " + "; ".join(failures))


def assert_preregistered_pool_dir(pool_dir: Path) -> None:
    resolved = pool_dir.resolve()
    if resolved != DEFAULT_POOL_DIR.resolve():
        raise ValueError(f"V4 requires preregistered pool directory: {DEFAULT_POOL_DIR}")
    for forbidden in (V1_POOL_DIR.resolve(), V2_POOL_DIR.resolve(), V3_POOL_DIR.resolve()):
        if resolved == forbidden or forbidden in resolved.parents:
            raise ValueError(f"V4 may not use prior steering source pools: {resolved}")
    ...
    *,
    ...
    for raw_path in paths:
        ...
        if path.name != "final_subjects.json":
            continue
        if allow_v4_final and path == V4_FINAL_RAW.resolve():
            continue
        raise ValueError(f"V4 steering final raw path is forbidden before final eval: {path}")
...
def generate_pools(args: argparse.Namespace, pool_dir: Path) -> Dict[str, Any]:
    ...
    for pool_name, pool_config in POOL_CONFIGS.items():
        ...
        payload["claim_scope"] = "four_behavior_representation_steering_v4_source_pool"
    ...
    final_redacted["claim_scope"] = "redacted_final_steering_v4_source_pool_audit_surface_only"
    ...
    audit["claim_scope"] = "four_behavior_representation_steering_v4_source_pool_construction"
    audit = redact_v4_combined_audit(audit)
...
def redact_v4_combined_audit(audit: Mapping[str, Any]) -> Dict[str, Any]:
...
def run_development(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths(paths.values(), allow_v4_final=False)
...
def run_final(args: argparse.Namespace, pool_dir: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    assert_no_forbidden_final_raw_paths([eval_path], allow_v4_final=True)
    payload = train_and_evaluate(
...
def train_and_evaluate(
    *,
    ...
    assert_no_forbidden_final_raw_paths(
        ...
        allow_v4_final=allow_eval_final_raw,
    )
    ...
    if hash_failures:
        raise ValueError("V4 source-pool file hash validation failed: " + "; ".join(hash_failures))
    ...
    transport_path = output_dir / "low_rank_residual_transport_stats.pt"
    torch.save(
        {
            "centroids": train_stats["centroids"],
            "inv_sqrt_cov": train_stats["inv_sqrt_cov"],
            "method": "train_low_rank_residual_covariance_transport_no_optimizer",
    ...
    result = {
        **eval_result,
        ...
        "claim_scope": (
            ...
            else "four_behavior_representation_steering_v4_low_rank_residual_transport_final"
        ),
...
def fit_train_statistics(subjects: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    if vh.size(0) < rank:
        raise ValueError(f"V4 requires at least {rank} principal components")
...
def evaluate_record(
    *,
    ...
    matched_z = apply_low_rank_residual_transport(
        z=z,
        source=source,
        target=target,
        stats=train_stats,
...
def build_control_candidate_specs(
    *,
    ...
    specs: list[Dict[str, Any]] = [
        ...
        candidate_spec(
            "reverse_low_rank_residual_transport",
    ...
    for other_target in PATTERNS:
        ...
        specs.append(candidate_spec(
            "same_source_other_target_low_rank_residual_transport",
    ...
    for other_source in PATTERNS:
        if other_source in (source, target):
            continue
        specs.append(candidate_spec(
            "same_target_other_source_low_rank_residual_transport",
    ...
    specs.append(candidate_spec(
        "shuffled_low_rank_residual_transport",
...
def validate_source_pool_contract(
    *,
    ...
    failures = []
    if combined_audit.get("claim_scope") != "four_behavior_representation_steering_v4_source_pool_construction":
        failures.append("combined source-pool audit is not V4-specific")
    if final_redacted.get("claim_scope") != "redacted_final_steering_v4_source_pool_audit_surface_only":
        failures.append("final redacted audit is not V4-specific")
    if train_payload.get("claim_scope") != "four_behavior_representation_steering_v4_source_pool":
        failures.append("train source pool is not V4-specific")
    if eval_payload.get("claim_scope") != "four_behavior_representation_steering_v4_source_pool":
        failures.append(f"{phase} eval source pool is not V4-specific")
    ...
    if not combined_audit.get("seed_preflight", {}).get("passed", False):
        failures.append("V4 seed-range preflight did not pass")
