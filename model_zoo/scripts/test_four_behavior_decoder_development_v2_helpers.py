"""Direct helper tests for four-behavior decoder development V2."""
...
import train_four_behavior_decoder_development_v2 as dev  # noqa: E402
...
def test_distillation_cases_exclude_all_heldout_sequences() -> None:
    suite = dev.build_suite()
    cases = dev.build_distillation_cases(suite)
    heldout = {
        tuple(seq)
        for pattern in dev.PATTERNS
        for split_name in ("positive", "negative")
        for seq in suite["heldout"][pattern][split_name]
    }
    assert len(cases) == 4096
    assert not (set(map(tuple, cases)) & heldout)
    assert dev.hash_sequences(cases) == dev.hash_sequences(dev.build_distillation_cases(suite))
...
def main() -> None:
    ...
    test_distillation_cases_exclude_all_heldout_sequences()
    print("v2 helper tests passed")
