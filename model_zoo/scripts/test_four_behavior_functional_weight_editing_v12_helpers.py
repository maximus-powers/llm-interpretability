"""Direct helper tests for V12 conflict-aware aligned editing."""
...
import train_four_behavior_functional_weight_editing_v12_conflict_aware_aligned_interpolation as v12  # noqa: E402


def test_v12_rejects_prior_v11_final_raw() -> None:
    prior = REPO_ROOT / "runs" / "four_behavior_functional_weight_editing_v11_pools" / "final_subjects.json"
    try:
        v12.assert_no_forbidden_final_raw_paths([prior], allow_v12_final=False)
    except ValueError as error:
        assert "prior sealed final raw path" in str(error)
    else:
        raise AssertionError("V12 accepted V11 final raw as an input")


def test_v12_rejects_own_final_before_authorization() -> None:
    try:
        v12.assert_no_forbidden_final_raw_paths([v12.V12_FINAL_RAW], allow_v12_final=False)
    except ValueError as error:
        assert "V12 final raw path is forbidden" in str(error)
    else:
        raise AssertionError("V12 accepted its final raw before authorization")


def test_v12_development_contract_does_not_hash_final_raw(monkeypatch) -> None:
    ...
    monkeypatch.setattr(v12.v1, "sha256_file", fake_sha256)
    counts_64 = {pattern: 64 for pattern in v12.PATTERNS}
    counts_24 = {pattern: 24 for pattern in v12.PATTERNS}
    combined_audit = {
        "claim_scope": v12.SOURCE_AUDIT_SCOPE,
        "overlap_counts": {},
        "passed": True,
        "pool_summaries": {
            "development": {
    ...
    final_redacted = {
        "claim_scope": v12.FINAL_REDACTED_SCOPE,
        "pool_file_sha256": "finalhash",
    }

    failures = v12.validate_source_pool_contract(
        ...
        train_payload={"claim_scope": v12.SOURCE_POOL_SCOPE},
        eval_payload={"claim_scope": v12.SOURCE_POOL_SCOPE},
...
def test_v12_contract_rejects_forbidden_final_public_detail_keys(monkeypatch) -> None:
    monkeypatch.setattr(v12.v1, "sha256_file", lambda path: {
        "development_subjects.json": "devhash",
        "train_subjects.json": "trainhash",
    }.get(path.name, "otherhash"))
    counts_64 = {pattern: 64 for pattern in v12.PATTERNS}
...
def test_v12_interpolates_in_raw_weight_space() -> None:
    source = torch.tensor([0.0, 10.0, -10.0])
    target = torch.tensor([100.0, -10.0, 30.0])
    edited = v12.interpolate_weights(source_weights=source, target_weights=target, alpha=0.975)
    assert torch.allclose(edited, torch.tensor([97.5, -9.5, 29.0]))


def test_v12_shuffled_target_uses_lexicographic_candidates(monkeypatch) -> None:
    monkeypatch.setattr(v12, "stable_hash_json", lambda payload: "0000000000000001")
    selected = v12.select_shuffled_target(
        subject_id="subject",
...
def test_v12_random_controls_are_raw_norm_matched_and_deterministic() -> None:
    source = torch.zeros(345, dtype=torch.float32)
    controls_a = v12.random_weight_delta_controls(
        subject_id="subject",
        source="sorted_ascending",
        target="has_majority",
    ...
    controls_b = v12.random_weight_delta_controls(
        subject_id="subject",
        source="sorted_ascending",
        target="has_majority",
        source_weights=source,
...
def test_v12_source_target_split_counts_match_preregistration() -> None:
    for direction, expected in v12.EXPECTED_SPLIT_COUNTS.items():
        source, target = direction.split("_to_")
        split = v12.source_target_split(source=source, target=target)
        assert split["compatible_count"] == expected["compatible"]
        assert split["conflict_count"] == expected["conflict"]


def test_v12_alignment_preserves_target_function_under_permutation() -> None:
    base = torch.arange(345, dtype=torch.float32) / 100.0
    layers = v12.unpack_subject_weights(base)
    ...
    target = v12.pack_subject_weights(weights, biases)

    aligned = v12.align_target_to_source(source_weights=base, target_weights=target)
    assert torch.allclose(aligned, base)


def test_v12_gate_failures_include_conflict_and_compatible_gates() -> None:
    aggregate = {
    ...
    records = [{
        "controls": [
            ...
            {"control_type": "unaligned_full_nearest_target_retrieval"},
            *[
                {"control_type": f"dummy:{index}"}
                for index in range(v12.THRESHOLDS["expected_controls_per_record"] - 2)
            ],
        ],
        "matched": {
            "compatible_count": v12.EXPECTED_SPLIT_COUNTS["sorted_descending_to_has_majority"]["compatible"],
            "conflict_count": v12.EXPECTED_SPLIT_COUNTS["sorted_descending_to_has_majority"]["conflict"],
        },
...
if __name__ == "__main__":
    test_v12_rejects_prior_v11_final_raw()
    test_v12_rejects_own_final_before_authorization()
    test_v12_interpolates_in_raw_weight_space()
    test_v12_random_controls_are_raw_norm_matched_and_deterministic()
    test_v12_source_target_split_counts_match_preregistration()
    test_v12_alignment_preserves_target_function_under_permutation()
    test_v12_gate_failures_include_conflict_and_compatible_gates()
    print("V12 direct helper tests passed; run pytest for fixture-based leak checks.")
