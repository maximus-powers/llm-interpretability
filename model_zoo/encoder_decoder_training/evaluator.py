...
def compute_reconstruction_metrics(predicted: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, config: Dict[str, Any]):
    if predicted.dim() == 3:
...
def compute_weight_level_metrics(reconstructed_weights: Dict[str, torch.Tensor], original_weights: Dict[str, torch.Tensor]):
    metrics = {}
    layer_metrics = []
    ...
    for layer_name in original_weights.keys():
        ...
        if orig.shape != recon.shape:
            logger.warning(f"Shape mismatch for {layer_name}: orig={orig.shape}, recon={recon.shape}")
            continue
...
def print_metrics(metrics: Dict[str, float], prefix: str = ""):
    ...
    main_metrics = ['loss', 'mse', 'mae', 'rmse', 'cosine_similarity', 'relative_error', 'r2_score']
    ...
    if 'layer_mse_mean' in metrics:
        logger.info("\n  Layer-Level Metrics:")
        logger.info(f"    MSE (mean ± std): {metrics['layer_mse_mean']:.6f} ± {metrics.get('layer_mse_std', 0):.6f}")
        logger.info(f"    MAE (mean ± std): {metrics['layer_mae_mean']:.6f} ± {metrics.get('layer_mae_std', 0):.6f}")
        logger.info(f"    Max Error (mean): {metrics.get('layer_max_error_mean', 0):.6f}")
    if 'per_layer' in metrics and metrics['per_layer']:
        logger.info("\n  Per-Layer Breakdown:")
        for layer_metric in metrics['per_layer'][:10]:
            logger.info(f"    {layer_metric['layer']}: "
