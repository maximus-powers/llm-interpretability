    ...
    def __getitem__(self, idx):
        ...
        if self.input_mode in ["signature", "both"] and "improved_signature" in example:
            ...
            signature_features, signature_mask = preprocess_signature(
                signature_json, self.max_dims, self.method_names
            )
...
def load_dataset(config: Dict[str, Any]):
    ...
    logger.info(
        f"Tokenizer created: chunk_size={tokenizer.chunk_size}, max_tokens={tokenizer.max_tokens}, token_dim={tokenizer.token_dim}"
    )
