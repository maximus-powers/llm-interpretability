...
class SubjectModel(nn.Module):
    ...
    def __init__(
        ...
        logger.debug(
            f"Created SubjectModel: {num_layers} layers, "
            f"{neurons_per_layer} neurons/layer, {activation_type} activation"
        )
        ...
        if quantization_type == "int8":  # scale to [-127, 127] range and round
            ...
            for param in self.parameters():
                if param.requires_grad:
                    threshold = (
                        param.abs().max() * 0.2
                    )  # if it's 20% of max away from 0, set to 0
            ...
            logger.warning(
                f"Unknown quantization type '{quantization_type}', skipping quantization"
            )
...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(self, examples, vocab=None, vocab_size=None):
        ...
        logger.debug(
            f"Created SequenceDataset with {len(examples)} examples, vocab: {self.vocab}"
        )
...
class SubjectModelTrainer:
    def __init__(self, device: str = "auto", quantization_type: str = "none"):
        ...
        logger.info(
            f"Initialized trainer with device: {self.device}, quantization: {quantization_type}"
        )
    ...
    def train_and_evaluate(
        ...
        epoch_offset: int = 0,
        wait_for_first_improvement: bool = False,
        ...
        if log_dir and tensorboard_config and tensorboard_config.get("enabled", False):
            try:
                ...
                if epoch_offset == 0:
                    ...
                    writer.add_text(
                        f"{pattern_combo}/stage_transition",
                        f"Transition from DEGRADED to IMPROVED stage at epoch {epoch_offset}",
                        epoch_offset,
                    )
                    writer.add_scalar(
                        f"{pattern_combo}/Stage/transition_marker", 1.0, epoch_offset
                    )
                    logger.info(
                        f"Continuing TensorBoard logging from epoch {epoch_offset}"
                    )
        ...
        try:
            for epoch in range(num_epochs):
                ...
                if (
                    wait_for_first_improvement
                    and initial_val_loss is not None
                    and not improved_at_least_once
                ):
                    improvement_pct = (
                        (initial_val_loss - avg_val_loss) / initial_val_loss
                    ...
                    if improvement_pct >= min_improvement_threshold:
                        ...
                        logger.info(
                            f"First significant improvement detected at epoch {epoch + 1} "
                        ...
                        if writer:
                            ...
                            writer.add_scalar(
                                f"{pattern_combo}/Markers/first_improvement",
                                1.0,
                                global_epoch_temp,
                            )
                            writer.add_text(
                                f"{pattern_combo}/first_improvement",
                        ...
                        logger.info(
                            f"Small improvement at epoch {epoch + 1}: {improvement_pct * 100:.1f}% "
                ...
                if val_acc > best_val_accuracy:
                    ...
                    logger.info(
                        f"New best accuracy: {val_acc:.4f} at epoch {epoch + 1}"
                    )
                    if writer:
                        global_epoch_temp = epoch + epoch_offset
                        writer.add_scalar(
                            f"{pattern_combo}/Markers/best_accuracy",
                            val_acc,
                            global_epoch_temp,
                        )
                        writer.add_text(
                            f"{pattern_combo}/best_accuracy",
                ...
                global_epoch = (
                    epoch + epoch_offset
                )  # offset = degraded epochs if in improvement stage
                ...
                if writer:
                    ...
                    writer.add_scalar(
                        f"{pattern_combo}/Loss/train", avg_train_loss, global_epoch
                    )
                    writer.add_scalar(
                        f"{pattern_combo}/Loss/val", avg_val_loss, global_epoch
                    )
                    writer.add_scalar(
                        f"{pattern_combo}/Accuracy/train", train_acc, global_epoch
                    )
                    writer.add_scalar(
                        f"{pattern_combo}/Accuracy/val", val_acc, global_epoch
                    )
                    writer.add_scalar(
                        f"{pattern_combo}/Learning_rate", learning_rate, global_epoch
                    )
                    stage_value = 0.0 if stage == "degraded" else 1.0
                    writer.add_scalar(
                        f"{pattern_combo}/Stage/current_stage",
                        stage_value,
                        global_epoch,
                    )
                ...
                if save_checkpoints and log_dir:
                    checkpoint_subdir = (
                        f"checkpoints_{stage}" if stage else "checkpoints"
                    )
                ...
                # log every epoch
                logger.info(
                    f"Epoch {epoch + 1}/{num_epochs} - Train Loss: {avg_train_loss:.4f}, Train Acc: {train_acc:.4f}, Val Loss: {avg_val_loss:.4f}, Val Acc: {val_acc:.4f}"
                )
                ...
                if wait_for_first_improvement and improved_at_least_once:
                    logger.info(
                        f"Returning early after first improvement at epoch {epoch + 1}"
                    )
                    break
            ...
            if writer and training_history:
                try:
                    ...
                    logger.info(
                        f"Logged hyperparameters to TensorBoard: {pattern_combination} (stage: {stage})"
                    )
    ...
    def train_staged_improvement(
        ...
        clean_train_loader,
        clean_val_loader,
        max_degraded_epochs: int,
        improvement_epochs: int,
        learning_rate: float,
        improvement_lr_factor: float = 0.1,
        early_stopping_patience: int = 10,
        degraded_log_dir: str = None,
        improved_log_dir: str = None,
        selected_patterns: List[str] = None,
        target_pattern: str = None,
        tensorboard_config: Dict[str, Any] = None,
        checkpoint_config: Dict[str, Any] = None,
        ...
        logger.info(
            f"Stage 1: Training on corrupted data (waiting for {min_improvement_threshold * 100:.1f}% improvement)..."
        )
        ...
        if not degraded_results["improved_at_least_once"]:
            logger.warning(
                f"Model never improved during degraded stage ({degraded_results['epochs_trained']} epochs)"
            )
            return {
                "success": False,
        ...
        logger.info(
            f"Stage 1 complete - First improvement at epoch {first_improvement_epoch + 1}, Val accuracy: {degraded_acc:.4f}"
        )
        ...
        improved_results = self.train_and_evaluate(
            ...
            tensorboard_config=tensorboard_config,
            checkpoint_config=checkpoint_config,
            epoch_offset=first_improvement_epoch
            + 1,  # start from first improvement epoch
        ...
        for idx, metrics in enumerate(improved_results["training_history"]):
            if (
                abs(metrics.get("val_acc", 0) - improved_acc) < 0.0001
        ...
        logger.info(
            f"Stage 2 complete - Best validation accuracy: {improved_acc:.4f} (epoch {best_epoch_offset}), Improvement: {improvement:+.4f}"
        )
...
def create_subject_model(
    ...
    vocab_size: int = 7,
    sequence_length: int = 7,
) -> Tuple[SubjectModel, Dict[str, Any]]:
    ...
    logger.info(
        f"Initialized subject model '{model_id}' with {num_layers} layers, {neurons_per_layer} neurons/layer"
    )
