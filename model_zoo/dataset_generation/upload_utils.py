...
def compute_aggregate_stats(
    metrics_dir: Path, valid_example_ids: set = None
) -> Dict[str, Any]:
    stats = {
    ...
    for example_dir in metrics_dir.glob("example_*"):
        ...
        try:
            ...
            logger.warning(
                f"Could not extract example ID from directory: {example_dir.name}"
            )
            continue
        ...
        try:
            ...
            if (
                "degraded_accuracy" in metadata
                and metadata["degraded_accuracy"] is not None
            ):
                ps["degraded_accuracy"].append(metadata["degraded_accuracy"])
            if (
                "degraded_epochs" in metadata
                and metadata["degraded_epochs"] is not None
            ):
            ...
            if (
                "improved_accuracy" in metadata
                and metadata["improved_accuracy"] is not None
            ):
                ps["improved_accuracy"].append(metadata["improved_accuracy"])
            if (
                "improved_epochs" in metadata
                and metadata["improved_epochs"] is not None
            ):
    ...
    if valid_example_ids is not None:
        logger.info(
            f"Computed aggregate stats from {stats['examples_in_metrics']} valid examples "
            f"(scanned {stats['examples_scanned']}, filtered {stats['examples_filtered']} discarded)"
        )
    else:
        logger.info(
            f"Computed aggregate stats from {stats['examples_in_metrics']} examples in metrics directory"
        )
...
def compute_token_stats(examples: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    token_stats = {"modification": {"counts": []}, "classification": {"counts": []}}
    ...
    for example in examples:
        if "modification_prompt" in example:
            try:
                count = len(
                    tokenizer.encode(
                        example["modification_prompt"], add_special_tokens=True
                    )
                )
        ...
        if "classification_prompt" in example:
            try:
                count = len(
                    tokenizer.encode(
                        example["classification_prompt"], add_special_tokens=True
                    )
                )
    ...
    logger.info(
        f"Computed token stats: {len(token_stats['modification']['counts'])} modification, "
        f"{len(token_stats['classification']['counts'])} classification prompts"
    )
    return token_stats


def _format_pattern_stats_table(
    pattern_stats: Dict[str, Dict], include_modification: bool
) -> str:
    ...
    for pattern_combo, stats in sorted(pattern_stats.items()):
        ...
        avg_degraded_loss = (
            sum(stats["degraded_loss"]) / len(stats["degraded_loss"])
            if stats["degraded_loss"]
            else 0
        )
        avg_degraded_acc = (
            sum(stats["degraded_accuracy"]) / len(stats["degraded_accuracy"])
            if stats["degraded_accuracy"]
            else 0
        )
        avg_degraded_epochs = (
            sum(stats["degraded_epochs"]) / len(stats["degraded_epochs"])
            if stats["degraded_epochs"]
            else 0
        )
        avg_improved_loss = (
            sum(stats["improved_loss"]) / len(stats["improved_loss"])
            if stats["improved_loss"]
            else 0
        )
        avg_improved_acc = (
            sum(stats["improved_accuracy"]) / len(stats["improved_accuracy"])
            if stats["improved_accuracy"]
            else 0
        )
        avg_improved_epochs = (
            sum(stats["improved_epochs"]) / len(stats["improved_epochs"])
            if stats["improved_epochs"]
            else 0
        )
        avg_improvement = (
            sum(stats["improvement"]) / len(stats["improvement"])
    ...
    if include_modification:
        overall_deg_loss = (
            sum(totals["degraded_loss"]) / len(totals["degraded_loss"])
            if totals["degraded_loss"]
            else 0
        )
        overall_deg_acc = (
            sum(totals["degraded_accuracy"]) / len(totals["degraded_accuracy"])
            if totals["degraded_accuracy"]
            else 0
        )
        overall_deg_epochs = (
            sum(totals["degraded_epochs"]) / len(totals["degraded_epochs"])
            if totals["degraded_epochs"]
            else 0
        )
        overall_imp_loss = (
            sum(totals["improved_loss"]) / len(totals["improved_loss"])
            if totals["improved_loss"]
            else 0
        )
        overall_imp_acc = (
            sum(totals["improved_accuracy"]) / len(totals["improved_accuracy"])
            if totals["improved_accuracy"]
            else 0
        )
        overall_imp_epochs = (
            sum(totals["improved_epochs"]) / len(totals["improved_epochs"])
            if totals["improved_epochs"]
            else 0
        )
        overall_improvement = (
            sum(totals["improvement"]) / len(totals["improvement"])
            if totals["improvement"]
            else 0
        )
...
def generate_dataset_card_content(
    ...
    if config["task_generation"].get("include_modification"):
        tasks.append(
            "- Modify the weights of a model to alter it's classification properties based on an activation signature, with examples of: degraded model + signature → improved weights."
        )
...
"""
    ...
    if token_stats:
        ...
        if mod_stats.get("counts"):
            token_rows.append(
                f"| Modification | {mod_stats['min']} | {mod_stats['max']} | {mod_stats['avg']:.1f} |"
            )
        if class_stats.get("counts"):
            token_rows.append(
                f"| Classification | {class_stats['min']} | {class_stats['max']} | {class_stats['avg']:.1f} |"
            )
...
"""
    ...
    if aggregate_stats and aggregate_stats.get("pattern_stats"):
        include_modification = config["task_generation"].get(
            "include_modification", False
        )
        pattern_table = _format_pattern_stats_table(
            aggregate_stats["pattern_stats"], include_modification
        )
    ...
    try:
        ...
        for i, example in enumerate(examples):
            ...
            if "modification_prompt" in example:
                ...
                formatted["modification_completion"] = example[
                    "modification_completion"
                ]
                formatted["modification_text"] = (
                    example["modification_prompt"] + example["modification_completion"]
                )

            if "classification_prompt" in example:
                formatted["classification_prompt"] = example["classification_prompt"]
                formatted["classification_completion"] = example[
                    "classification_completion"
                ]
                formatted["classification_text"] = (
                    example["classification_prompt"]
                    + example["classification_completion"]
                )

            # add signature fields if present
            if "degraded_signature" in example:
                formatted["degraded_signature"] = json.dumps(
                    example["degraded_signature"]
                )

            if "improved_signature" in example:
                formatted["improved_signature"] = json.dumps(
                    example["improved_signature"]
                )

            # add model weight fields if present
            if "degraded_model_weights" in example:
                formatted["degraded_model_weights"] = json.dumps(
                    example["degraded_model_weights"]
                )

            if "improved_model_weights" in example:
                formatted["improved_model_weights"] = json.dumps(
        ...
        try:
            ...
            if metrics_dir:
                ...
                for example in new_dataset["train"]:
                    try:
                        ...
                        logger.debug(
                            f"Could not extract example_id from example metadata: {e}"
                        )
                aggregate_stats = compute_aggregate_stats(
                    metrics_dir, valid_example_ids
                )
            token_stats = compute_token_stats(new_dataset["train"])
            card_content = generate_dataset_card_content(
                config, aggregate_stats, token_stats
            )
            ...
            try:
                ...
                if signature_dataset_path.exists():
                    ...
                    logger.warning(
                        f"Signature dataset not found at {signature_dataset_path}, skipping upload"
                    )
            ...
            logger.warning(
                "Dataset was uploaded successfully, but card generation failed"
            )
