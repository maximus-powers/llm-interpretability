...
class ActivationSignatureExtractor:
    ...
    def __init__(
        self,
        device: str = "auto",
        signature_dataset_path: str = None,
        neuron_profile_config: Dict[str, Dict] = None,
    ):
        if not signature_dataset_path.exists():
            raise FileNotFoundError(
                f"Signature dataset not found at {signature_dataset_path}. Create one using pattern_sampler.create_signature_dataset_file(), and make sure you hold onto it. This is the element used for interpretability, and the same dataset must be used for all training examples and inference."
            )
        ...
        self.neuron_profile_config = neuron_profile_config or {"mean": {}, "std": {}}
    ...
    def extract(self, model: SubjectModel, batch_size: int = 32) -> Dict[str, Any]:
        ...
        logger.info(
            f"Extracting features from model using {len(self.signature_dataset['examples'])} baseline examples"
        )
        ...
        )  # no shuffling for consistency in activations, shouldn't matter tho
        ...
        with torch.no_grad():
            for (
                ...
                _labels,
                _patterns,
            ) in loader:  # SequenceDataset now returns (data, labels, patterns)
        ...
        processed_features = self._process_layer_activations(
            layer_activations, example_patterns
        )
        features = {
    ...
    def _compute_pca(
        self, all_layer_activations: np.ndarray, neuron_id: int, components: int = 1
    ) -> List[float]:
        ...
        try:
            ...
            n_components_actual = min(
                components,
                all_layer_activations.shape[0] - 1,
                all_layer_activations.shape[1],
            )
    ...
    def _compute_clustering(
        self, neuron_activations: np.ndarray, n_clusters: int = 2
    ) -> List[float]:
        """K-means cluster centers - reveals distinct activation states (e.g., on/off modes)"""
        if (
            len(np.unique(neuron_activations)) < n_clusters
        ):  # not enough unique values to form clusters
        ...
        try:
            ...
            return [
                float(np.min(neuron_activations)),
                float(np.max(neuron_activations)),
            ]
    ...
    def _compute_svd(
        ...
        try:
            ...
            result = Vt[
                :components, neuron_id
            ].tolist()  # coefficients for this neuron in the top singular vectors
            return result
        except Exception:
            return [0.0] * components

    def _compute_fourier(
        self, neuron_activations: np.ndarray, n_frequencies: int = 1
    ) -> List[float]:
    ...
    def _compute_pattern_wise(
        self, neuron_activations: np.ndarray, example_patterns: List[str]
    ) -> List[float]:
        """Mean activation for each pattern in the signature dataset"""
        if len(neuron_activations) != len(example_patterns):
            logger.warning(
                f"Mismatch between activations ({len(neuron_activations)}) and patterns ({len(example_patterns)})"
            )
            return []
            ...
            for neuron_id in range(num_neurons):
                ...
                for method, params in self.neuron_profile_config.items():
                    if method == "mean":
                        ...
                        profile["entropy"] = self._compute_entropy(
                            neuron_activations, bins
                        )
                    elif method == "clustering":
                        n_clusters = params.get("n_clusters", 2)
                        profile["clustering"] = self._compute_clustering(
                            neuron_activations, n_clusters
                        )
                    elif method == "svd":
                        components = params.get("components", 1)
                        ...
                        profile["fourier"] = self._compute_fourier(
                            neuron_activations, n_frequencies
                        )
