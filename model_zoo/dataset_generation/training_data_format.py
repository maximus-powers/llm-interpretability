...
class TrainingDataFormatter:
    ...
    def create_training_example(
        ...
        if include_modification:
            ...
            example["modification_completion"] = self._serialize_model_weights(
                target_model
            )
            ...
            if input_model is not None:
                example["degraded_model_weights"] = self._serialize_model_with_config(
                    input_model
                )
        ...
        if include_classification:
            ...
            example["classification_completion"] = (
                ", ".join(actual_patterns) if actual_patterns else ""
            )
            ...
            if improved_model is not None:
                example["improved_model_weights"] = self._serialize_model_with_config(
                    improved_model
                )
    ...
    def _build_modification_prompt(
        ...
        if self.format_style == "separate":
            ...
            sections.extend(
                self._format_signature_section(
                    baseline_features, "Activation Signature"
                )
            )
        else:  # interwoven
            sections.extend(
                self._format_interwoven_weights_and_signature(
                    input_model, baseline_features
                )
            )
    ...
    def _build_classification_prompt(
        self,
        improved_model: SubjectModel,
        improved_signature: Dict[str, Any],
        all_pattern_descriptions: Dict[str, str],
    ) -> str:
        sections = []
        ...
        if self.format_style == "separate":
            ...
            sections.extend(
                self._format_signature_section(
                    improved_signature, "Activation Signature"
                )
            )
        else:  # interwoven
            sections.extend(
                self._format_interwoven_weights_and_signature(
                    improved_model, improved_signature
                )
            )

        # add task section
        sections.extend(
            self._format_classification_task_section(all_pattern_descriptions)
        )
    ...
    def _format_layer_signature_data(
        self, layer_name: str, layer_data: Dict[str, Any], indent: str = ""
    ) -> List[str]:
        sections = []

        if not isinstance(layer_data, dict):
            raise ValueError(
                f"layer_data must be a dict, got {type(layer_data)} for {layer_name}"
            )

        if "neuron_profiles" not in layer_data:
            raise ValueError(
                f"layer_data must contain 'neuron_profiles' key for {layer_name}, got keys: {layer_data.keys()}"
            )
                ...
                for v in values:
                    if isinstance(v, (list, np.ndarray)):
                        formatted_values.append(
                            "["
                            + ", ".join([f"{x:.{self.precision}f}" for x in v])
                            + "]"
                        )
                    else:
    ...
    def _format_signature_section(
        self, signature: Dict[str, Any], title: str
    ) -> List[str]:
        sections = [f"## {title}", ""]

        if "neuron_activations" not in signature:
            raise ValueError(
                f"signature must contain 'neuron_activations' key, got keys: {signature.keys()}"
            )
    ...
    def _format_interwoven_weights_and_signature(
        self, model: SubjectModel, signature: Dict[str, Any]
    ) -> List[str]:
        ...
        for layer_name in sorted(layer_weights.keys()):
            ...
            if layer_num in neuron_activations:
                ...
                sections.extend(
                    self._format_layer_signature_data(
                        layer_num, layer_data, indent="  "
                    )
                )
    ...
    def _format_classification_task_section(
        self, all_pattern_descriptions: Dict[str, str]
    ) -> List[str]:
    ...
    def _serialize_model_with_config(self, model: SubjectModel) -> Dict[str, Any]:
        ...
        return {"config": model.config, "weights": weights_data}
