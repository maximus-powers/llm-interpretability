...
class DatasetGenerationPipeline:
    ...
    def __init__(self, config: dict, example_id_setter=None):
        ...
        if not self.include_modification and not self.include_classification:
            raise ValueError(
                "At least one task type must be enabled in task_generation config"
            )

        logger.info(
            f"DatasetGenerationPipeline initialized with tasks: modification={self.include_modification}, classification={self.include_classification}"
        )
    ...
    def _validate_config(self):
        ...
        if enabled_patterns is not None:
            ...
            invalid_patterns = [
                p for p in enabled_patterns if p not in available_patterns
            ]
            if invalid_patterns:
                raise ValueError(
                    f"Invalid patterns in configuration: {invalid_patterns}.\nAvailable patterns: {available_patterns}"
                )
            if len(enabled_patterns) == 0:
                raise ValueError(
                    "enabled_patterns cannot be an empty list. Use null to enable all patterns."
                )
            min_patterns_per_batch = self.config["dataset"]["patterns"][
                "min_patterns_per_batch"
            ]
            if len(enabled_patterns) < min_patterns_per_batch:
                raise ValueError(
                    f"Number of enabled patterns ({len(enabled_patterns)}) is less than min_patterns_per_batch ({min_patterns_per_batch})"
                )
            logger.info(
                f"Pattern validation passed: {len(enabled_patterns)} patterns enabled"
            )
        else:
        ...
        if format_style not in valid_styles:
            raise ValueError(
                f"Invalid prompt format style: '{format_style}'. Must be one of: {valid_styles}"
            )

        logger.info(f"Prompt format validation passed: using '{format_style}' style")

    def generate_training_examples(
        self, num_examples: int = None, min_degradation: float = None
    ):
        ...
        logger.info(
            f"Starting independent training example generation: {num_examples} examples"
        )
        ...
        if checkpoint_data:
            logger.info(
                f"Resuming from checkpoint: {checkpoint_data['total_generated']} examples already generated"
            )
            start_from = checkpoint_data["total_generated"]
            remaining_examples = num_examples - start_from
            if remaining_examples <= 0:
                logger.info(
                    "Target number of examples already reached according to checkpoint"
                )
                return []
        else:
            start_from = 0
        ...
        with ThreadPoolExecutor(max_workers=self.max_threads) as executor:
            ...
            while total_generated + len(completed_examples) < num_examples:
                ...
                while len(futures) < self.max_threads and (
                    total_generated + len(completed_examples) < num_examples
                ):
                    future = executor.submit(
                        self._generate_single_example, example_id, min_degradation
                    )
                    ...
                    try:
                        ...
                        if example and example.get("is_valid", False):
                            ...
                            with self._logging_lock:
                                current_progress = total_generated + len(
                                    completed_examples
                                )
                                logger.info(
                                    f"EXAMPLE {current_progress}/{num_examples} completed (improvement: {example.get('metadata', {}).get('improvement', 0):.3f})"
                                )
                    except Exception as e:
                        with self._logging_lock:
                            logger.error(
                                f"Example {futures[future]} generation failed: {e}"
                            )
                ...
                if (
                    ...
                    and not self.upload_only_at_end
                    and len(completed_examples) >= self.checkpoint_interval
                ):
                    with self._checkpoint_lock:
                        logger.info(
                            f"Checkpoint triggered: Saving {len(completed_examples)} new examples"
                        )
                        try:
                            ...
                            logger.info(
                                f"Checkpoint saved successfully: {current_total} total examples"
                            )
                ...
                if total_generated + len(completed_examples) >= num_examples:
                    logger.info(
                        f"Target reached: {total_generated + len(completed_examples)}/{num_examples} examples"
                    )
                    break
        ...
        logger.info(
            f"Generated {len(completed_examples)} new training examples (total: {total_generated})"
        )

        return completed_examples

    def _generate_single_example(
        self, example_id: int, min_degradation: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            available_patterns = [
                p
                for p in self.pattern_sampler.patterns.keys()
            ...
            num_patterns = thread_random.randint(
                min_patterns, min(max_patterns, len(available_patterns))
            )
            selected_patterns = thread_random.sample(available_patterns, num_patterns)

            # create dataset
            target_total_examples = self.config["dataset"]["patterns"][
                "target_total_examples"
            ]
            max_total_examples = self.config["dataset"]["patterns"][
                "max_total_examples"
            ]
            negative_ratio = self.config["dataset"]["patterns"]["negative_ratio"]
            min_samples_per_pattern = self.config["dataset"]["patterns"][
                "samples_per_pattern"
            ]["min"]

            target_positives = int(target_total_examples / (1 + negative_ratio))
            examples_per_pattern = max(
                min_samples_per_pattern, target_positives // num_patterns
            )
            ...
            corruption_rate = self.config.get("staged_training", {}).get(
                "corruption_rate", 0.5
            )
            corrupted_dataset = self._corrupt_dataset(
                clean_dataset,
                target_pattern,
            ...
            with self._logging_lock:
                logger.info(
                    f"Example {example_id}: {num_patterns} patterns, corrupting '{target_pattern}'"
                )

            # run staged training
            staged_results = self._train_staged_model(
                corrupted_dataset,
                clean_dataset,
                model_config,
                target_pattern,
            ...
            if not staged_results.get("improvement", 0) >= min_degradation:
                with self._logging_lock:
                    logger.info(
                        f"Example {example_id}: Insufficient improvement ({staged_results.get('improvement', 0):.4f} < {min_degradation})"
                    )
            ...
            if self.include_modification:
                ...
                degraded_signature = self.activation_signature_extractor.extract(
                    degraded_model
                )
            ...
            if self.include_classification or self.include_modification:
                ...
                if self.include_classification:
                    improved_signature = self.activation_signature_extractor.extract(
                        improved_model
                    )
            ...
            if self.include_classification:
                enabled_patterns = self.config["dataset"]["patterns"][
                    "enabled_patterns"
                ]
                all_pattern_descriptions = {
                    pattern: descriptions.get(pattern) for pattern in enabled_patterns
                }

            # aggregate training metrics from both stages
            training_metrics = self._aggregate_training_metrics(
                staged_results, selected_patterns
            )
            ...
            example = self.interpreter_formatter.create_training_example(
                ...
                degraded_signature=degraded_signature
                if self.include_modification
                else None,
                improved_signature_data=improved_signature
                if self.include_classification
                else None,
            ...
            with self._logging_lock:
                logger.info(
                    f"Example {example_id} completed in {generation_time / 60:.1f}min (improvement: {staged_results['improvement']:.4f})"
                )
            ...
            with self._logging_lock:
                logger.error(
                    f"Example {example_id} failed after {generation_time / 60:.1f}min: {e}"
                )
    ...
    def _train_staged_model(
        self,
        corrupted_dataset: Dict[str, Any],
        ...
        num_workers = (
            0 if self.max_threads > 1 else self.config["pipeline"].get("num_workers", 0)
        )
        if num_workers != self.config["pipeline"].get("num_workers", 0):
            with self._logging_lock:
                logger.info(
                    f"Disabling DataLoader workers (set to 0) because threading is enabled (max_threads={self.max_threads})"
                )
        ...
        tensorboard_enabled = metrics_config.get("tensorboard", {}).get(
            "enabled", False
        )
        ...
        if tensorboard_enabled and self.metrics_dir:
            ...
            example_prefix = (
                ...
                else f"example_{int(time.time())}"
            )
            patterns_str = "_".join(
                sorted(corrupted_dataset.get("target_patterns", []))[:3]
            )  # First 3 patterns
            target_pattern_short = target_pattern[:8] if target_pattern else "unknown"
            log_dir = (
                self.metrics_dir
                / f"{example_prefix}_target-{target_pattern_short}_patterns-{patterns_str}"
            )
        ...
        staged_results = self.model_trainer.train_staged_improvement(
            ...
            max_degraded_epochs=self.config.get("staged_training", {}).get(
                "max_degraded_epochs", 10
            ),
            improvement_epochs=self.config.get("staged_training", {}).get(
                "improvement_epochs", 10
            ),
            learning_rate=model_config["learning_rate"],
            improvement_lr_factor=self.config.get("staged_training", {}).get(
                "improvement_lr_factor", 0.1
            ),
            ...
            min_improvement_threshold=self.config.get("staged_training", {}).get(
        ...
        if tensorboard_enabled and self.metrics_dir and degraded_log_dir:
            try:
                ...
                metadata["degraded_accuracy"] = staged_results["degraded"].get(
                    "accuracy", None
                )
                metadata["degraded_epochs"] = degraded_metrics.get(
                    "epochs_trained", None
                )
                metadata["improved_loss"] = improved_metrics.get("best_val_loss", None)
                metadata["improved_accuracy"] = staged_results["improved"].get(
                    "accuracy", None
                )
                metadata["improved_epochs"] = improved_metrics.get(
                    "epochs_trained", None
                )
                ...
                with self._logging_lock:
                    logger.warning(
                        f"Failed to update metadata with training results: {e}"
                    )
    ...
    def _corrupt_dataset(
        ...
        thread_random: random.Random = None,
    ) -> Dict[str, Any]:
        ...
        if not target_examples:
            with self._logging_lock:
                logger.warning(
                    f"No examples found for pattern '{target_pattern}' to corrupt"
                )
        ...
        target_indices = [
            i for i, ex in enumerate(examples) if ex.get("pattern") == target_pattern
        ]
        ...
        corrupted_dataset["corruption_stats"] = {
            ...
            "actual_corruption_rate": corrupted_count / len(target_examples)
            if target_examples
            else 0,
        }
        ...
        with self._logging_lock:
            logger.info(
                f"Corrupted {corrupted_count}/{len(target_examples)} examples of pattern '{target_pattern}' ({corrupted_dataset['corruption_stats']['actual_corruption_rate']:.1%})"
            )
        return corrupted_dataset

    def _aggregate_training_metrics(
        self, staged_results: Dict[str, Any], selected_patterns: List[str]
    ):
        degraded_history = staged_results["degraded"]["metrics"].get(
            "training_history", []
        )
        improved_history = staged_results["improved"]["metrics"].get(
            "training_history", []
        )
        combined_history = []
        ...
        return {
            ...
            "summary": {
                ...
                "degraded_stage": {
                    "initial_val_loss": degraded_history[0]["val_loss"]
                    if degraded_history
                    else None,
                    "final_val_loss": degraded_history[-1]["val_loss"]
                    if degraded_history
                    else None,
                    "initial_val_acc": degraded_history[0]["val_acc"]
                    if degraded_history
                    else None,
                    "final_val_acc": degraded_history[-1]["val_acc"]
                    if degraded_history
                    else None,
                    "best_val_acc": staged_results["degraded"]["accuracy"],
                },
                "improved_stage": {
                    "initial_val_loss": improved_history[0]["val_loss"]
                    if improved_history
                    else None,
                    "final_val_loss": improved_history[-1]["val_loss"]
                    if improved_history
                    else None,
                    "initial_val_acc": improved_history[0]["val_acc"]
                    if improved_history
                    else None,
                    "final_val_acc": improved_history[-1]["val_acc"]
                    if improved_history
                    else None,
                ...
                "improvement": staged_results["improvement"],
                "first_improvement_epoch": staged_results.get(
    ...
    def _generate_model_config(
        self, thread_random: random.Random = None, batch_id: int = 0
    ) -> Dict[str, Any]:
        if thread_random is None:
            thread_random = random
        ...
        num_layers = thread_random.randint(
            model_config["num_layers"]["min"], model_config["num_layers"]["max"]
        )
        neurons_per_layer = thread_random.randint(
        ...
        learning_rate = thread_random.uniform(
            model_config["learning_rate"]["min"], model_config["learning_rate"]["max"]
        )
        return {
            # architecture
    ...
    def save_checkpoint(
        self, total_generated: int, completed_examples: List[Dict[str, Any]]
    ):
        checkpoint_data = {
    ...
    def load_checkpoint(self) -> Optional[Dict[str, Any]]:
        if self.checkpoint_file.exists():
            try:
                ...
                logger.info(
                    f"Checkpoint loaded: {checkpoint_data['total_generated']} examples previously generated"
                )
