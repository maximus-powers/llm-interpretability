...
def compute_dimensions_from_config(config: Dict[str, Any]):
    ...
    result = {
        ...
        "max_total_params": input_layer_params
        + hidden_layer_params
        + output_layer_params,
...
def infer_signature_dimensions(
    signature_json: str, method_names: List[str]
) -> Dict[str, Any]:
...
def compute_model_architecture(
    input_dims: Dict[str, int], num_patterns: int, config: Dict[str, Any] = None
) -> Dict[str, Any]:
    ...
    if model_config.get("signature_encoder", {}).get("hidden_dims"):
        ...
        logger.info(
            f"Using custom signature_encoder hidden_dims from config: {sig_hidden}"
        )
    else:
    ...
    if model_config.get("weight_encoder", {}).get("hidden_dims"):
        ...
        logger.info(
            f"Using custom weight_encoder hidden_dims from config: {weight_hidden}"
        )
    else:
    ...
    # fusion layer
    fusion_input = (sig_hidden[-1] if sig_hidden else 0) + (
        weight_hidden[-1] if weight_hidden else 0
    )
    fusion_hidden = None
...
def preprocess_signature(
    ...
    for layer_idx_str, layer_data in neuron_activations.items():
        ...
        if layer_idx >= max_total_linear_layers:
            logger.warning(
                f"Layer index {layer_idx} exceeds max_total_linear_layers {max_total_linear_layers}"
            )
            continue
        ...
        for neuron_idx_str, profile in neuron_profiles.items():
            ...
            if neuron_idx >= max_dims["max_neurons_per_layer"]:
                logger.warning(
                    f"Neuron index {neuron_idx} exceeds max_neurons {max_dims['max_neurons_per_layer']}"
                )
                continue
...
def preprocess_weights(weights_json: str, max_total_params: int):
    ...
    if actual_count > max_total_params:
        logger.warning(
            f"Actual params {actual_count} exceeds max {max_total_params}, truncating"
        )
...
class PatternClassifierDataset(Dataset):
    def __init__(
        ...
        logger.info(
            f"Initialized PatternClassifierDataset: {len(self.hf_dataset)} examples, "
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        if self.input_mode in ["weights", "both"]:
            weights, weight_mask = preprocess_weights(
                example["improved_model_weights"], self.max_dims["max_total_params"]
            )
...
def load_dataset(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not method_names:
        raise ValueError(
            "neuron_profile.methods is empty. Must specify at least one method to extract."
        )
...
def create_dataloaders(dataset_info: Dict[str, Any], config: Dict[str, Any]):
    ...
    train_dataset = PatternClassifierDataset(
        train_dataset, max_dims, input_mode, all_patterns, method_names
    )
    val_dataset = PatternClassifierDataset(
        val_dataset, max_dims, input_mode, all_patterns, method_names
    )
    test_dataset = PatternClassifierDataset(
        test_dataset, max_dims, input_mode, all_patterns, method_names
    )
