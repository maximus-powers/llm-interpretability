...
class PatternClassifierMLP(nn.Module):
    def __init__(self, config: Dict[str, Any], input_dims: Dict[str, int]):
        ...
        logger.info(
            f"Initializing PatternClassifierMLP with input_mode={self.input_mode}, batch_norm={self.use_batch_norm}"
        )
        ...
        if self.input_mode in ["signature", "both"]:
            if "signature" not in input_dims:
                raise ValueError(
                    f"Input mode '{self.input_mode}' requires 'signature' in input_dims"
                )
            ...
            logger.info(
                f"Signature encoder ({len(sig_hidden_dims)} layers): {input_dims['signature']} -> {' -> '.join(map(str, sig_hidden_dims))}"
            )
        ...
        if self.input_mode in ["weights", "both"]:
            if "weights" not in input_dims:
                raise ValueError(
                    f"Input mode '{self.input_mode}' requires 'weights' in input_dims"
                )
            ...
            logger.info(
                f"Weight encoder ({len(weight_hidden_dims)} layers): {input_dims['weights']} -> {' -> '.join(map(str, weight_hidden_dims))}"
            )
        ...
        logger.info(
            f"Fusion ({len(fusion_hidden_dims)} layers): {fusion_in} -> {' -> '.join(map(str, fusion_hidden_dims))} -> {num_patterns} patterns"
        )
    ...
    def _build_mlp(
        self, input_dim: int, hidden_dims: List[int], dropout: float, activation: str
    ) -> nn.Sequential:
        layers = []
        activations = {
            "relu": nn.ReLU(),
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.input_mode in ["signature", "both"]:
            if "signature" not in inputs:
                raise ValueError(
                    f"Input mode '{self.input_mode}' requires 'signature' in inputs"
                )
        ...
        if self.input_mode in ["weights", "both"]:
            if "weights" not in inputs:
                raise ValueError(
                    f"Input mode '{self.input_mode}' requires 'weights' in inputs"
                )
