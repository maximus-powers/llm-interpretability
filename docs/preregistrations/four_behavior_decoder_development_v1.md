# Four-Behavior Stored-Probe Decoder Development V1 Preregistration

Date: 2026-06-10

## Purpose

This preregistration freezes the train/development phase for a four-behavior
stored-probe signature-to-weight decoder.
...
- heldout-excluded support behavior BCE for the subject's target behavior;
- source-margin hinge loss on support cases, margin target `0.30`.

Loss weights:

- reconstruction MSE: `1.0`;
- support BCE: `0.2`;
- support margin hinge: `0.5`.

No differentiable stored-probe signature reconstruction loss is allowed in this
V1 development run because the current registered extractor detaches weights and
runs under `no_grad`.

## Model Selection

Evaluate on the development pool every `25` epochs.
...
`noise_seed = int(first_16_hex(stable_hash_json([development_subject_id, "noise", 20260612])), 16) % 2**31`
...
For the matched decode, also report reconstruction MSE against the source
weights.
